...
function hydrateFarm(row: { ownerId: string; code: string; name: string; saveData: any }): Farm {
  ...
  const tiles: Tile[] =
    Array.isArray(s.tiles) && s.tiles.length === TILE_COUNT ? s.tiles : freshTiles();
  return {
    id: row.ownerId,
    code: row.code,
    name: row.name,
    ...
    members:
      Array.isArray(s.members) && s.members.length
        ? s.members
        : [{ userId: row.ownerId, name: 'Farmer' }],
    joinRequests: [],
    presence: new Map(),
...
function season(farm: Farm): Season {
  return SEASONS[farm.seasonIndex];
}
...
function currentFarm(
  socket: Socket,
): { farm: Farm; userId: string; name: string; p: PresenceRec } | null {
  const st = socketState.get(socket.id);
  if (!st) return null;
  const farm = farms.get(st.farmId);
  if (!farm) return null;
...
function onPlant(socket: Socket, payload: any): void {
  ...
  const t = farm.tiles[idx(tx, tz)];
  if (t.terrain !== 1) {
    toast(socket, 'Till the soil first.', 'error');
    return;
  }
  if (t.crop) {
    toast(socket, 'Something is already planted here.', 'error');
    return;
  }
  ...
  if ((farm.seeds[cropId] ?? 0) <= 0) {
    toast(socket, `No ${def.name} seeds.`, 'error');
    return;
  }
  if (!spendEnergy(farm, p, socket, ENERGY_COST)) return;
...
function onHarvest(socket: Socket, payload: any): void {
  ...
  for (const i of areaTiles(tx, tz, radius)) {
    ...
    const key =
      mult >= 2 ? `${crop.cropId}#gold` : mult >= 1.5 ? `${crop.cropId}#silver` : crop.cropId;
...
function onBuy(socket: Socket, payload: any): void {
  ...
  if (farm.money < cost) {
    toast(socket, 'Not enough money.', 'error');
    return;
  }
  farm.money -= cost;
...
function onUpgradeTool(socket: Socket, payload: any): void {
  ...
  if (!isHost(farm, userId)) {
    toast(socket, 'Only the farm host can upgrade equipment.', 'error');
    return;
  }
  ...
  if (!next) {
    toast(socket, 'Already at max level.', 'error');
    return;
  }
  if (farm.money < next.cost) {
    toast(socket, 'Not enough money.', 'error');
    return;
  }
...
function onSleep(socket: Socket): void {
  ...
  const { farm, userId } = ctx;
  if (!isHost(farm, userId)) {
    toast(socket, 'Only the host can end the day (sleep).', 'error');
    return;
  }
  ...
  for (const s of socketsInFarm(farm)) {
    s.emit(S2C.TOAST, {
      message: `Day ${farm.day} — ${season(farm)}, ${farm.weather}.`,
      kind: 'info',
    });
  }
...
async function onJoinFarm(socket: Socket, payload: any): Promise<void> {
  ...
  const code = String(payload?.code || '')
    ...
    .replace(/[^A-Z0-9]/g, '');
  if (!code) {
    err(socket, 'Enter a farm code.');
    return;
  }
  const farm = await loadFarmByCode(code);
  if (!farm) {
    err(socket, 'No farm found with that code.');
    return;
  }
  if (socket.disconnected) return;
...
export function registerRmhFarmingSimHandlers(io: Server, socket: Socket): void {
  ...
  socket.on(C2S.JOIN_FARM, (p) => {
    void onJoinFarm(socket, p);
  });
  socket.on(C2S.APPROVE_JOIN, (p) => onApproveJoin(socket, p));
  socket.on(C2S.DENY_JOIN, (p) => onDenyJoin(socket, p));
  socket.on(C2S.KICK, (p) => {
    void onKick(socket, p);
  });
  socket.on(C2S.LEAVE_FARM, () => {
    void onLeaveFarm(socket);
  });
