...
/**
 * `/sohumbum2` — the live activity dossier.
 *
 ...
 * and the unfurl is real) and then polls `/api/sohumbum2/activity`. Every
...
import { formatDayLong, isoWeekKey, monthKeyOf } from '@/lib/sohumbum2/dates';
import type { WatchStateDTO, WatchSummaryDTO } from '@/lib/sohumbum2/types';
...
interface Sohumbum2PageProps {
  initialState: WatchStateDTO;
  /** How many days the loader asked for; the poller keeps asking for the same. */
  historyDays: number;
  /** Deep-linked day, when the visitor arrived on a `/sohumbum2/<date>` URL. */
  initialSelectedKey?: string | null;
}

export function Sohumbum2Page({
  initialState,
  historyDays,
  initialSelectedKey = null,
}: Sohumbum2PageProps) {
  const { t } = useTranslation('r-sohumbum2');
  const { state, refreshing, stale } = useWatchState(initialState, historyDays);
    ...
    <div className="sb2">
      <div className="sb2-shell">
        <header className="sb2-header">
          <span className="sb2-header__mark" aria-hidden>
            <Activity size={18} />
          </span>
          <h1 className="sb2-header__title">
            {t('title', { defaultValue: 'What Is Sohum Doing Right Now?' })}
            <span className="sb2-header__sub">
              {t('subtitle', {
          ...
          <div className="sb2-header__actions">
            {/* A quiet, honest freshness indicator rather than a spinner that
                implies the page is broken while it refetches. */}
            <span className="sb2-section__note" aria-live="polite">
              {stale
                ? t('feed-stale', { defaultValue: 'Reconnecting…' })
                : refreshing
                  ? t('feed-updating', { defaultValue: 'Updating…' })
                  : t('feed-live', { defaultValue: 'Live' })}
            </span>
            <RefreshCw
              aria-hidden
              size={14}
              className={refreshing ? 'sb2-live-clock' : undefined}
              style={{ color: 'var(--sb2-text-faint)' }}
            />
          </div>
        </header>
        ...
        {state.empty ? (
          <section className="sb2-section">
            <div className="sb2-empty">
              <p className="sb2-empty__title">
                {t('empty-title', { defaultValue: 'Nothing recorded yet' })}
              </p>
              <p className="sb2-empty__body">
                {t('empty-body', {
                  defaultValue:
          ...
          <>
            <section className="sb2-section" aria-labelledby="sb2-figures">
              <div className="sb2-section__head">
                <h2 className="sb2-section__title" id="sb2-figures">
            ...
            <section className="sb2-section" aria-labelledby="sb2-charts">
              <div className="sb2-section__head">
                <h2 className="sb2-section__title" id="sb2-charts">
            ...
            <section className="sb2-section" aria-labelledby="sb2-calendar">
              <div className="sb2-section__head">
                <h2 className="sb2-section__title" id="sb2-calendar">
                  {t('calendar-heading', { defaultValue: 'The calendar' })}
                </h2>
                <p className="sb2-section__note">
                  {t('calendar-note', {
              ...
              <div className="sb2-grid-2">
                <div className="sb2-card sb2-card--pad">
                  <ActivityCalendar
                    monthKey={monthKey}
                    days={days}
            ...
            <section className="sb2-section" aria-labelledby="sb2-periods">
              <div className="sb2-section__head">
                <h2 className="sb2-section__title" id="sb2-periods">
...
function Stat({
  ...
  return (
    <div className={alarm ? 'sb2-stat sb2-stat--alarm' : 'sb2-stat'}>
      <span className="sb2-stat__label">{label}</span>
      <span className="sb2-stat__value">{value}</span>
      <span className="sb2-stat__note">{note}</span>
    </div>
  );
}
