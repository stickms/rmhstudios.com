...
/**
 * The charts on `/sohumbum2`, drawn as plain SVG.
 *
...
import { formatDayShort } from '@/lib/sohumbum2/dates';
...
export function ActivityCharts({ days }: ChartsProps) {
  ...
  return (
    <div className="sb2-grid-2">
      <div className="sb2-chart">
        <div className="sb2-chart__head">
          <h3 className="sb2-chart__title">
            {t('chart-trend-title', { defaultValue: 'Day by day' })}
          </h3>
          <div
            className="sb2-segmented"
            role="group"
      ...
      <div className="sb2-chart">
        <div className="sb2-chart__head">
          ...
          <div className="sb2-chart__legend">
            <span
              className="sb2-chart__key"
              style={{ '--sb2-swatch': 'var(--sb2-accent)' } as React.CSSProperties}
            >
              <span className="sb2-chart__swatch" aria-hidden />
              {t('legend-voice', { defaultValue: 'Voice' })}
            </span>
            <span
              className="sb2-chart__key"
              style={{ '--sb2-swatch': 'var(--sb2-online)' } as React.CSSProperties}
            >
              <span className="sb2-chart__swatch" aria-hidden />
    ...
    <>
      ...
      >
        {[0.5, 1].map((fraction) => (
          <line
            key={fraction}
            className="sb2-chart__grid-line"
            x1={PAD_L}
            x2={PLOT_W - PAD_R}
        ...
        <text className="sb2-chart__axis" x={0} y={PAD_T + 8}>
          {render(max)}
        </text>
        ...
        {days.length > 0 ? (
          <>
            <text className="sb2-chart__axis" x={PAD_L} y={PLOT_H - 6}>
              {formatDayShort(days[0].dateKey)}
            </text>
            <text
              className="sb2-chart__axis"
              x={PLOT_W - PAD_R}
              y={PLOT_H - 6}
              textAnchor="end"
            >
      ...
      <p className="sb2-chart__readout" aria-live="polite">
        {activeDay ? (
          <>
      ...
      >
        ...
        {messages.map((value, hour) => {
          ...
          return (
            <rect
              key={`m-${hour}`}
              className="sb2-chart__bar sb2-chart__bar--alt"
