...
/**
 * The month calendar — the spine of `/sohumbum2`.
 *
...
export function ActivityCalendar({
  ...
  return (
    <div>
      <div className="sb2-calendar__nav">
        <button
          type="button"
        ...
        <h3 className="sb2-calendar__month">{formatMonthLong(monthKey)}</h3>
        <button
          type="button"
          className="sb2-btn sb2-btn--ghost sb2-btn--icon"
      ...
      <div className="sb2-calendar__grid">
        {dayNames.map((name) => (
          <div key={name} className="sb2-calendar__dow" aria-hidden>
            {name}
          </div>
        ))}

        {Array.from({ length: leading }, (_, i) => (
          <div key={`blank-${i}`} className="sb2-day sb2-day--blank" aria-hidden />
        ))}
        ...
        {cells.map((dateKey) => {
          ...
          if (isFuture) {
            return (
              <div key={dateKey} className="sb2-day sb2-day--future" aria-hidden>
                <span className="sb2-day__num">{Number(dateKey.slice(8, 10))}</span>
              </div>
            );
          }
          ...
          return (
            <button
              ...
              style={{ '--sb2-heat': `var(--sb2-heat-${level})` } as React.CSSProperties}
              onClick={() => onSelect(dateKey)}
              aria-label={label}
              aria-pressed={dateKey === selectedKey}
            >
              <span className="sb2-day__num" aria-hidden>
                {Number(dateKey.slice(8, 10))}
              </span>
              <span className="sb2-day__voice" aria-hidden>
                {voiceSec > 0 ? formatDuration(voiceSec) : '—'}
              </span>
              <span className="sb2-day__msgs" aria-hidden>
                {day?.messages ? formatCount(day.messages) : ''}
              </span>
              {day?.summary ? <span className="sb2-day__mark" aria-hidden /> : null}
            </button>
          );
        })}
      </div>

      <p className="sb2-legend">
        <span>{t('cal-legend-less', { defaultValue: 'Less' })}</span>
        {[0, 1, 2, 3, 4].map((level) => (
          <span
            key={level}
            className="sb2-legend__cell"
            style={{ '--sb2-heat': `var(--sb2-heat-${level})` } as React.CSSProperties}
            aria-hidden
          />
        ))}
