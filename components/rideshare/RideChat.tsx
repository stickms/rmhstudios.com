...
export function RideChat({ messages, currentUserId, onSend, disabled, otherName }: RideChatProps) {
  ...
  return (
    <div className="flex flex-col rounded-xl border border-site-border bg-site-surface/40">
      ...
      <div ref={scrollRef} className="max-h-56 min-h-32 flex-1 space-y-2 overflow-y-auto p-3">
        {messages.length === 0 ? (
          <p className="py-8 text-center text-xs text-site-text-muted">
            {disabled ? 'Chat is unavailable.' : 'Say hi 👋 — messages are private to this trip.'}
          </p>
        ) : (
          messages.map((m) => {
      ...
      <form onSubmit={submit} className="flex items-center gap-2 border-t border-site-border p-2.5">
        <input
          ...
          className="min-w-0 flex-1 rounded-lg border border-site-border bg-site-surface px-3 py-2 text-sm text-site-text outline-none transition-colors placeholder:text-site-text-dim focus:border-site-accent/60 disabled:opacity-50"
        />
        <button
          type="submit"
