...
export const ExportWeatherData = ({ data }: { data: any }) => {
  ...
  return (
    <div className="my-2">
      <div className="text-lg font-semibold text-blue-400 mb-2">{t("export-weather-data", { defaultValue: "Export Weather Data" })}</div>
      <div className="bg-weather-glass rounded-2xl p-4 border border-weather text-weather flex flex-col gap-3">
        <p className="text-xs text-weather-muted">{t("download-description", { defaultValue: "Download current weather data for this location." })}</p>
        <div className="flex gap-2">
          ...
          >
            {t("export-json", { defaultValue: "Export JSON" })}
          </button>
        </div>
      </div>
    </div>
