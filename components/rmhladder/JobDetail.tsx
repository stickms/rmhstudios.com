...
export function JobDetail({
  ...
  const location =
    ...
    t('ladder.locationMissing', { defaultValue: 'Location not listed' });
  ...
  const verificationEvidence =
    typeof verification?.evidence === 'string' ? verification.evidence : null;
  ...
  const trackingActions = [
    {
      ...
      label: t('ladder.save', { defaultValue: 'Save job' }),
      icon: Bookmark,
    },
    {
    ...
    },
    {
      ...
      label: t('ladder.ignore', { defaultValue: 'Ignore job' }),
      icon: EyeOff,
    },
  ];
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      <Link
        to="/rmhladder/jobs"
        className="inline-flex min-h-11 items-center text-sm font-medium text-site-accent hover:underline"
      >
      ...
      <Card className="gap-5">
        <CardHeader className="gap-4">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="success">
              ...
              {t('ladder.verifiedRole', { defaultValue: 'Verified role' })}
            </Badge>
          ...
          <div>
            ...
            <p className="mt-2 font-medium text-site-text-muted">
              {company?.name ?? t('ladder.unknownCompany', { defaultValue: 'Unknown company' })}
            </p>
          </div>
        ...
        <CardContent className="space-y-6">
          <section
            aria-labelledby="ladder-match-heading"
            className="rounded-site-sm border border-site-border bg-site-bg p-4"
          >
            <h2 id="ladder-match-heading" className="text-sm font-semibold text-site-text">
              {t('ladder.roleRelevance', { defaultValue: 'Role relevance' })}
            </h2>
          ...
          {verification && (
            <section aria-labelledby="ladder-verification-heading">
              <h2 id="ladder-verification-heading" className="text-sm font-semibold text-site-text">
                {t('ladder.verification', { defaultValue: 'Verification' })}
              </h2>
              {verificationEvidence && (
                <p className="mt-2 text-sm leading-6 text-site-text-muted">
                  {verificationEvidence}
                </p>
              )}
              {verificationCheckedAt && (
                <p className="mt-1 text-xs text-site-text-dim">
                  {t('ladder.lastVerified', {
                    defaultValue: 'Last verified {{time}}',
                    time: timeAgo(verificationCheckedAt as Date),
                  })}
                </p>
              )}
            </section>
          ...
          {summary && (
            <section aria-labelledby="ladder-summary-heading">
              <h2 id="ladder-summary-heading" className="text-sm font-semibold text-site-text">
                {t('ladder.summary', { defaultValue: 'Role summary' })}
              </h2>
              <p className="mt-2 whitespace-pre-line text-sm leading-7 text-site-text-muted">
                {summary}
              </p>
            </section>
          )}
            ...
            <Button asChild size="lg" className="sm:mr-auto">
              ...
              >
                {t('ladder.applyOn', {
                  ...
                  domain: sourceDomain(applyUrl),
                })}
                <ExternalLink aria-hidden />
              </a>
            </Button>
          ) : (
            <p className="text-sm text-site-danger sm:mr-auto">
              {t('ladder.applyUnavailable', {
                defaultValue: 'The source application link is unavailable.',
              })}
            </p>
          )}
