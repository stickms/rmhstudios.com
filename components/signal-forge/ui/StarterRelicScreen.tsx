  ...
  return (
    <div className="w-full h-full bg-black bg-opacity-75 flex items-center justify-center z-50 p-6">
      <div className="bg-linear-to-b from-slate-900 to-black border-2 border-purple-500 p-8 rounded-lg max-w-3xl w-full shadow-2xl">
        <h2 className="text-3xl font-bold text-purple-400 mb-6 text-center">Choose Your Starter Relic</h2>
        <p className="text-slate-400 text-center mb-6">Select a relic to begin your run with:</p>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {choices.map((relic) => (
            ...
            >
              ...
              <p className="text-xs text-slate-400 capitalize mb-2">{relic.rarity} Relic</p>
              <p className="text-sm text-slate-300">{relic.description}</p>
            </button>
          ))}
        </div>
        <p className="text-slate-500 text-xs text-center">Floor {floor}</p>
      </div>
    </div>
  );
}
