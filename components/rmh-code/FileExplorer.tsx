...
import { ChevronDown, Plus, Trash2, FilePlus, FolderPlus } from 'lucide-react';
...
function FileIcon({ filename }: { filename: string }) {
  ...
  return (
    <span className={`text-xs mr-1.5 shrink-0 ${colorMap[ext] ?? 'text-[#858585]'}`}>
      ◆
    </span>
  );
}
...
export default function FileExplorer({
  projects, activeProject, files, activeFileId,
  onSelectProject, onNewProject, onDeleteProject,
  onOpenFile, onNewFile, onDeleteFile,
}: FileExplorerProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#252526] text-[#ccc] select-none min-w-0">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden text-xs">
        {!activeProject && (
          <div className="px-4 py-6 text-[#858585] text-center text-xs leading-relaxed">
        ...
        {activeProject && files.length === 0 && (
          <div className="px-4 py-4 text-[#858585] text-center text-xs">
            No files yet.{' '}
            <button onClick={onNewFile} className="text-[#007acc] hover:underline">
              Create one
            </button>
          </div>
        )}
        ...
        {root.map(file => (
          <div
            ...
            className={`flex items-center gap-1 px-3 py-0.5 cursor-pointer group transition-colors ${
          ...
          <div key={folder}>
            ...
            {expandedFolders.has(folder) && folderFiles.map(file => (
              <div
                ...
                onMouseLeave={() => setHoveredFile(null)}
                onClick={() => onOpenFile(file)}
                className={`flex items-center gap-1 pl-7 pr-3 py-0.5 cursor-pointer group transition-colors ${
                  activeFileId === file.id
                    ? 'bg-[#37373d] text-white'
                    : 'hover:bg-[#2a2d2e] text-[#ccc]'
                }`}
