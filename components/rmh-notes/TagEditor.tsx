...
export default function TagEditor({ noteTagIds, allTags, onChange, onClose }: Props) {
  ...
  const createAndAdd = () => {
    ...
    onChange(next);
    setNewTagName('');
    toast.success(t("tag-created", { defaultValue: 'Tag "{{name}}" created', name: tag.name }));
  };

  return (
    <div className="px-4 py-2.5" style={{ background: 'var(--notes-surface-2)', borderBottom: '1px solid var(--notes-border)' }}>
      <div className="flex items-center gap-2 flex-wrap mb-2">
        <span className="text-xs font-medium" style={{ color: 'var(--notes-text-muted)' }}>{t("tags-label", { defaultValue: "Tags:" })}</span>
        {allTags.map((tag) => (
          <button
            key={tag.id}
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          placeholder={t("new-tag-placeholder", { defaultValue: "New tag..." })}
        ...
        >
          {t("add-tag-button", { defaultValue: "+ Tag" })}
        </button>
