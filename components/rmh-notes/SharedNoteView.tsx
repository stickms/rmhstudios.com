  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#FEF9F0' }}>
      <div className="text-center max-w-md p-8">
        ...
        <h1 className="text-xl font-bold mb-2" style={{ color: '#2C1F0F' }}>{t("sharing-not-available", { defaultValue: "Sharing not available" })}</h1>
        <p style={{ color: '#7A6451' }}>
          {t("sharing-offline-message", { defaultValue: "This app runs in offline mode. Shared note links are not supported. Notes are stored locally in your browser." })}
        </p>
        <a href="/secret/notes" className="inline-block mt-4 px-4 py-2 rounded-lg text-sm font-semibold" style={{ background: '#C17F3A', color: '#FFFDF8' }}>
          {t("go-to-rmhnotes", { defaultValue: "Go to RMHNotes" })}
        </a>
      </div>
    </div>
  );
