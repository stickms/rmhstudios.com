  ...
  return (
    <div className="bp-team-col" style={{ ['--c' as string]: color }}>
      <div className="bp-team-head">
        <span>{title}</span>
        <span className="bp-team-count">{players.length}/5</span>
      </div>
        ...
        {Array.from({ length: 5 }).map((_, i) => {
          ...
          if (!p) {
            ...
            return (
              <button
                key={i}
                className={`bp-slot empty ${canJoin ? 'joinable' : ''}`}
                disabled={!canJoin}
                onClick={() => roomClient.selectTeam(team)}
              >
                {canJoin ? '+ JOIN' : '—'}
              </button>
            );
          ...
          return (
            <div key={p.id} className={`bp-slot filled ${p.id === roomClient.id ? 'me' : ''}`}>
              ...
              <div className="bp-slot-info">
                <div className="bp-slot-name">
                  {p.name}
                  {p.isHost ? ' 👑' : ''}
                </div>
                <div className="bp-slot-agent" style={{ color: ag.color }}>
                  {ag.name} · {ROLE_LABEL[ag.role]}
                </div>
              </div>
              <span className={`bp-slot-ready ${p.ready ? 'on' : ''}`}>
                {p.ready ? 'READY' : '…'}
              </span>
            </div>
          );
...
export function Lobby() {
  ...
  useEffect(() => {
    ...
    roomClient
      ...
      .then(() => {
        ...
        setStatus('browser');
        subs.push(
          roomClient.on('roomList', (d) => setRooms((d as { rooms: RoomListItem[] }).rooms || [])),
        );
        subs.push(
          roomClient.on('lobby', (d) => {
            setRoom((d as { room: RoomState }).room);
            setStatus('room');
          }),
        );
        subs.push(
          roomClient.on('error', (d) => {
            const m = (d as { message: string }).message;
            if (/host left/i.test(m)) setStatus('browser');
          }),
        );
        subs.push(
          roomClient.on('start', (d) => {
  ...
  useEffect(() => {
    if (status === 'room') roomClient.selectAgent(selectedAgent);
  ...
  const startMatch = (payload: StartPayload) => {
    ...
    const config: MatchConfig = {
      ...
      humans: payload.players.map((p) => ({
        ...
        name: p.name,
        agentId: p.agentId,
        team: payload.mode === 'zombies' ? 'attackers' : p.team,
        isLocal: p.id === meId,
      })),
  ...
  const changeAgent = () => {
    setAgentReturn('lobby');
    setPhase('agentSelect');
  };
  const playOffline = () => {
    buildPracticeConfig('standard');
    setPhase('match');
  };
  ...
  if (status === 'connecting')
    return (
      <div className="bp-lobby">
        <div className="bp-loading">CONNECTING…</div>
      </div>
    );
  if (status === 'error') {
    return (
      <div className="bp-lobby">
        <div className="bp-menu-inner" style={{ textAlign: 'center', margin: 'auto' }}>
          <div className="bp-lock-title" style={{ fontSize: 36 }}>
            SERVERS UNREACHABLE
          </div>
          <div className="bp-result-actions" style={{ justifyContent: 'center', marginTop: 16 }}>
            <button className="bp-cta" onClick={playOffline}>
              PLAY OFFLINE VS BOTS
            </button>
            <button className="bp-cta bp-cta-ghost" onClick={() => setPhase('menu')}>
              BACK
            </button>
          </div>
        </div>
    ...
    return (
      <div className="bp-lobby">
        <div className="bp-lobby-nav">
          <button className="bp-back" onClick={() => setPhase('menu')}>
            ← MENU
          </button>
          <div className="bp-lobby-eventpass">FIND A MATCH</div>
          <button className="bp-back" onClick={changeAgent}>
        ...
        <div className="bp-browser">
          <div className="bp-browser-actions">
            <button className="bp-cta" onClick={() => roomClient.quickJoin(name, selectedAgent)}>
              ⚡ QUICK PLAY
            </button>
            <div className="bp-create">
              <div className="bp-create-row">
                <div className="bp-pillrow bp-pillrow-sm">
                  <button
                    className={createMode === 'standard' ? 'on' : ''}
                    onClick={() => setCreateMode('standard')}
                  >
                    STANDARD
                  </button>
                  <button
                    className={createMode === 'zombies' ? 'on' : ''}
                    onClick={() => setCreateMode('zombies')}
                  >
                    ZOMBIES
                  </button>
                </div>
                <button
                  className={`bp-toggle ${createPrivate ? 'on' : ''}`}
                  onClick={() => setCreatePrivate((v) => !v)}
                >
                  {createPrivate ? '🔒 PRIVATE' : '🌐 PUBLIC'}
                </button>
              </div>
              {createPrivate && (
                <input
              ...
              <button
                className="bp-cta bp-cta-ghost"
                onClick={() =>
                  roomClient.createRoom({
                    name,
                    agentId: selectedAgent,
                    roomName: `${name}'s Lobby`,
                    isPublic: !createPrivate,
                    password: createPrivate && createPass ? createPass : null,
                    mode: createMode,
                  })
                }
              >
          ...
          <div className="bp-roomlist">
            <div className="bp-roomlist-head">
              PUBLIC LOBBIES{' '}
              <button className="bp-back" onClick={() => roomClient.listRooms()}>
                ↻ REFRESH
              </button>
            </div>
            {rooms.length === 0 && (
              <div className="bp-roomlist-empty">
                No public lobbies yet — Quick Play or create one.
              </div>
            )}
            {rooms.map((r) => (
              <div key={r.id} className="bp-room-row">
                <span className="bp-room-name">
                  {r.hasPassword ? '🔒 ' : ''}
                  {r.name}
                </span>
                <span className="bp-room-mode">{r.mode === 'zombies' ? 'ZOMBIES' : '5V5'}</span>
                <span className="bp-room-count">
                  {r.count}/{r.max}
                </span>
                ...
                {joinPass?.id === r.id ? (
                  <span className="bp-room-joinpass">
                    <input
                      ...
                      autoFocus
                      value={joinPass.pass}
                      onChange={(e) => setJoinPass({ id: r.id, pass: e.target.value })}
                    />
                    <button
                      className="bp-mini-btn"
                      onClick={() => roomClient.joinRoom(r.id, name, selectedAgent, joinPass.pass)}
                    >
                      GO
                    </button>
                  </span>
                  ...
                  <button
                    className="bp-mini-btn"
                    disabled={r.state === 'playing' || r.count >= r.max}
                    onClick={() => {
                      if (r.hasPassword) setJoinPass({ id: r.id, pass: '' });
                      else roomClient.joinRoom(r.id, name, selectedAgent, null);
                    }}
                  >
  ...
  return (
    <div className="bp-lobby">
      <div className="bp-lobby-nav">
        <button className="bp-back" onClick={leave}>
          ← LEAVE
        </button>
        <div className="bp-lobby-eventpass">
          {room?.name ?? 'LOBBY'} · {isZombies ? 'ZOMBIES' : 'COMPETITIVE'}
          {room?.isPublic ? '' : ' 🔒'}
        </div>
        <button className="bp-back" onClick={changeAgent}>
          AGENT: {getAgent(selectedAgent).name} ›
        ...
        <div className="bp-lobby-modebar">
          ...
          <div className="bp-pillrow bp-pillrow-sm">
            <button
              ...
              onClick={() => roomClient.setMode('standard')}
            >
              STANDARD 5V5
            </button>
            <button className={isZombies ? 'on' : ''} onClick={() => roomClient.setMode('zombies')}>
              ZOMBIES
            </button>
          </div>
          ...
          <div className="bp-pillrow bp-pillrow-sm">
            {[1, 2, 3, 4, 5].map((n) => (
              <button
                key={n}
                className={(room?.cpuPerSide ?? 4) === n ? 'on' : ''}
                onClick={() => roomClient.setCpu(n)}
              >
                {n}
              </button>
            ))}
      ...
      <div className="bp-lobby-bar">
        <button
          className={`bp-lobby-btn ${me?.ready ? 'ready-on' : ''}`}
          onClick={() => roomClient.ready(!me?.ready)}
        >
          {me?.ready ? '✓ READY' : 'READY UP'}
        </button>
        {isHost ? (
          <button
            className="bp-lobby-play"
            onClick={() => {
              setStarting(true);
              roomClient.start();
            }}
        ...
        <button className="bp-lobby-btn" onClick={leave}>
          LEAVE
        </button>
      </div>
      ...
      <div className="bp-lobby-hint">
        {isZombies
          ? 'Up to 5 survivors. Buy between waves and hold out.'
          : 'Click an empty slot to switch teams. Empty slots fill with AI (host sets team size).'}
      </div>

      <AnimatePresence>
        {starting && (
          <motion.div
            className="bp-matchfound"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
