  ...
  return (
    <div className="flex h-screen w-full flex-col items-center justify-center gap-6 bg-black p-8 text-center">
      <h2 className="text-2xl font-bold text-white">
        {gameName
          ? t('game-crashed-named', { gameName, defaultValue: '{{gameName}} crashed' })
          : t('game-crashed', { defaultValue: 'Game crashed' })}
      </h2>
...
export class GameErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error(
      `[GameErrorBoundary${this.props.gameName ? `: ${this.props.gameName}` : ''}]`,
      error,
      info.componentStack,
    );
