...
export type InboxTab = 'messages' | 'groups' | 'notifications';
 ...
 *
 * The active tab is **controlled by the route**, which mirrors it into `?tab=`.
 * It used to be local state seeded from an `initialTab` prop, and that quietly
 * broke the surfaces that now point here: /notifications redirects to
 * `?tab=notifications`, and arriving from an inbox page that was already
 * mounted changes only the search param — no remount, so a seeded-once state
 * would have stayed on Messages.
 */
export function InboxColumn({
  ...
  tab: InboxTab;
  onTabChange: (next: InboxTab) => void;
