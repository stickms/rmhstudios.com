...
function systemIdentity(entityType: string | null): {
 label: string;
 Icon: typeof Heart;
 tint: string;
 bg: string;
 ...
 label:'Achievement',
 Icon: Trophy,
 tint:'text-site-warning',
 bg:'bg-site-warning/15',
 };
 case'level':
 ...
 label:'RMH Rideshare',
 Icon: Car,
 tint:'text-site-success',
 bg:'bg-site-success/15',
 };
 case'ride_request':
 return {
 label:'New ride request',
 Icon: MapPin,
 tint:'text-site-accent',
 bg:'bg-site-accent/15',
 };
 case'ride_message':
 return {
 label:'Ride message',
...
function PreferencesPanel() {
 ...
 return (
 // Floating slab (§8.4): a discrete bg-site-surface border border-site-border rounded-site shadow-site-sm card over the aurora gutter.
 <div className="bg-site-surface border border-site-border rounded-site shadow-site-sm rounded-site mx-3 mt-3 px-4 py-3">
 ...
 <label
 key={row.field}
 className="flex cursor-pointer items-center justify-between gap-2 rounded-site-sm px-2 py-1.5 text-sm text-site-text hover:bg-site-surface-hover"
 >
 ...
 defaultValue:
'Applies to in-app and push notifications. Moderation notices are always delivered.',
 })}
 </p>
 </div>
 );
...
export function NotificationsColumn({
 ...
 toast.success(
 t('push-disabled', {
 defaultValue:'Push notifications turned off on this device.',
 }),
 );
 } else {
 ...
 toast.success(
 t('push-enabled', {
 defaultValue:"You'll now get push notifications on this device.",
 }),
 );
 else
 toast.error(
 t('push-enable-failed', {
 defaultValue:'Could not enable push — check browser permissions.',
 }),
 );
 }
 }}
 ...
 aria-label={t('notification-prefs-toggle', {
 ...
 {t('no-notifications-yet', { defaultValue:'No notifications yet'})}
 </p>
...
"When someone likes, comments on, reposts, or mentions you, it'll show up here.",
 ...
 <Reveal
 as="ul"
 className="bg-site-surface border border-site-border rounded-site shadow-site-sm rounded-site mx-3 mt-3 divide-y divide-site-border overflow-hidden"
 >
 {groups.map((g) => {
 const n = g.newest;
 ...
 {sys ? (
 <span
 className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${sys.bg}`}
 >
 ...
 {n.preview ||
 (sys
 ? t(`system-label-${n.entityType ??'default'}`, {
 defaultValue: sys.label,
 })
 :'')}
 </p>
 ) : (
 ...
 {t(`notification-verb-${n.type.toLowerCase()}`, {
 defaultValue: meta.verb,
 })}
 </span>
 </p>
 {n.preview && (
 ...
 {!g.read && (
 <span className="mt-2 h-2 w-2 shrink-0 rounded-full bg-site-accent"/>
 )}
 </button>
 </li>
 );
