...
export function ThreadSummary({ postId, commentCount }: { postId: string; commentCount: number }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (res.status === 503) {
        setError('Summaries are unavailable right now.');
        return;
      }
      const data = await res.json();
      if (data.summary) setSummary(data.summary);
      else setError('Not enough to summarize yet.');
    } catch {
      setError('Could not summarize this thread.');
    } finally {
      setLoading(false);
    }
  };
      ...
      {!requested ? (
        ...
        <div>
          <p className="mb-1 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-site-text-dim">
            <Sparkles className="h-3.5 w-3.5 text-site-accent" /> AI summary
          </p>
