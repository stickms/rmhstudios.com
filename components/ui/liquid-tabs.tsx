 ...
 *
 * Constraints:
 * - The capsule carries `.` (ambient sheen) — it IS a signature
...
export function LiquidTabs({
 ...
 >
 <motion.span
 className=" absolute inset-0 rounded-full bg-site-accent-dim shadow-[inset_0_1px_0_var(--site-glass-rim)]"
 style={squashStyle}
 />
 </motion.span>
 ) : null;
 ...
 // hairline glint edge comes from .bg-site-surface border border-site-border). The wrapper is presentational
 // only — the roving nav + layoutId capsule stay on the inner tablist/nav.
 return (
 <div
 data-slot="liquid-tabs-sheet"
 className={cn(
 'bg-site-surface border border-site-border min-w-0 max-w-full rounded-full p-1',
