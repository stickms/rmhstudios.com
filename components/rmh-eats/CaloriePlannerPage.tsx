...
export default function CaloriePlannerPage() {
    ...
    function handleSet() {
        ...
        if (isNaN(n) || n < 100) {
            toast.error('Please enter a valid calorie target (min 100)');
            return;
        }
        setCalorieBudget(n);
        toast.success(`Daily calorie budget set to ${n} cal`);
    }
    ...
    return (
        <div className="max-w-lg mx-auto space-y-5 pb-8">
            <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                    <h2 className="text-xl font-bold text-white">Calorie Planner</h2>
                    <p className="text-sm text-slate-400">Track your daily calorie intake</p>
                </div>
            </div>
            ...
            <div className="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-5">
                ...
                <div className="flex gap-2">
                    ...
                    />
                    <span className="flex items-center text-sm text-slate-400 pr-1">cal</span>
                    <button
                        onClick={handleSet}
                ...
                <>
                    ...
                    >
                        ...
                        <div className="flex justify-between items-end mb-3">
                            <div>
                                ...
                                <p className="text-xs text-slate-500">of {calorieBudget} cal</p>
                            </div>
                        ...
                        <p className="text-xs text-slate-500 text-right">{progressPct.toFixed(0)}% used</p>

                        {progressPct >= 100 && (
                            <div className="mt-3 rounded-xl bg-red-500/10 border border-red-500/30 px-3 py-2 text-xs text-red-400">
                                You've reached your daily calorie budget for today.
                            </div>
                        )}
                        {todayCalories === 0 && (
                            <p className="text-xs text-slate-500 mt-2">
                                No delivered orders today yet — your intake will appear here.
                            </p>
                        )}
                    </motion.div>
                    ...
                    {remaining! > 0 && withinBudgetItems.length > 0 && (
                        ...
                        >
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <h3 className="font-semibold text-white">Browse Within Budget</h3>
                                <span className="text-xs text-slate-400">({remaining} cal remaining)</span>
                            </div>
                    ...
                    {remaining! <= 0 && (
                        <div className="rounded-2xl bg-red-500/10 border border-red-500/30 p-5 text-center">
                            ...
                            <p className="font-semibold text-red-400 mt-3">Budget Reached</p>
                            <p className="text-sm text-slate-400 mt-1">
                                You've hit your {calorieBudget} cal daily budget. Your budget resets tomorrow.
                            </p>
                        </div>
                    )}
                </>
