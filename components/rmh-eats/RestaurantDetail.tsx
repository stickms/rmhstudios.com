    ...
    if (!restaurant) {
        return (
            <div className="flex flex-col items-center gap-4 py-24">
                ...
                <p className="text-slate-400">{t("restaurant-not-found", { defaultValue: "Restaurant not found." })}</p>
                <button onClick={goHome} className="text-orange-400 hover:text-orange-300 text-sm">
                    {t("go-back-home", { defaultValue: "Go back home" })}
                </button>
            </div>
        );
    }
        ...
        <div className="flex flex-col gap-0">
                ...
                <div className="p-5">
                    ...
                    <div className="mt-3 flex flex-wrap gap-4 text-sm text-slate-400">
                        ...
                        <span className="flex items-center gap-1.5">
                            ...
                            {restaurant.deliveryFee === 0 ? t("free-delivery", { defaultValue: "Free delivery" }) : t("delivery-fee", { defaultValue: "${{fee}} delivery", fee: restaurant.deliveryFee.toFixed(2) })}
                        </span>
                        <span className="flex items-center gap-1.5">
                    ...
                    <p className="mt-2 text-xs text-slate-500">
                        🕐 {restaurant.hours} · {t("min-order", { defaultValue: "Min. order ${{amount}}", amount: restaurant.minimumOrder })}
                    </p>
                </div>
            </div>
            ...
            {allAllergens.length > 0 && (
                <div className="mb-4 flex items-start gap-2 rounded-xl bg-yellow-500/10 border border-yellow-500/30 px-3 py-2.5">
                    ...
                    <p className="text-xs text-yellow-300">
                        <span className="font-semibold">{t("allergens-on-menu", { defaultValue: "Allergens on this menu: " })}</span>
                        {allAllergens.join(', ')}
                    </p>
                </div>
            )}
            ...
            <div className="sticky top-0 z-20 bg-slate-950 pt-2 pb-3 -mx-1 px-1">
                ...
                {calorieBudget != null && (
                    <div className={`mb-2 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium border ${
                        ...
                        {remainingCalories! >= 0
                            ? t("cal-remaining", { defaultValue: "{{count}} cal remaining today", count: remainingCalories })
                            : t("cal-over-budget", { defaultValue: "{{count}} cal over budget", count: Math.abs(remainingCalories!) })}
                    </div>
                )}
                ...
                <div className="relative mb-3">
                    ...
                    <input
                        type="text"
                        placeholder={t("search-menu", { defaultValue: "Search menu..." })}
                        value={search}
            ...
            {Object.keys(groupedItems).length === 0 ? (
                <div className="flex flex-col items-center gap-3 py-12 text-slate-500">
                    ...
                    <p>{t("no-items-match", { defaultValue: "No items match your search." })}{activeDietFilters.length > 0 ? t("or-active-filters", { defaultValue: " or active filters" }) : ''}</p>
                </div>
            ) : (
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {overBudget && (
                            <TriangleAlert className="h-3.5 w-3.5 text-orange-400 shrink-0" aria-label={t("over-calorie-budget", { defaultValue: "Over calorie budget" })} />
                        )}
                    </div>
                ...
                <p className={`text-xs mt-1 ${overBudget ? 'text-orange-400' : 'text-slate-500'}`}>
                    {item.calories} cal{overBudget && remainingCalories != null ? ` · ${t("item-over-budget", { defaultValue: "{{count}} over budget", count: item.calories - remainingCalories })}` : ''}
                </p>
            </div>
        </motion.button>
    );
