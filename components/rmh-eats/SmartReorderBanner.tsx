...
export default function SmartReorderBanner({ restaurantId }: Props) {
    ...
    function handleReorder() {
        ...
        if (added > 0) {
            toast.success(t("items-added-to-cart", { count: added, defaultValue: "{{count}} item(s) added to cart!" }), { icon: '🛒' });
        }
    }
    ...
    return (
        <div className="mx-0 mb-4 rounded-2xl bg-orange-500/10 border border-orange-500/30 px-4 py-3 flex items-center gap-3">
            <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-orange-400">{t("welcome-back", { defaultValue: "Welcome back!" })}</p>
                <p className="text-xs text-slate-400 mt-0.5 truncate">
                    {t("last-time-preview", { preview, defaultValue: "Last time: {{preview}}" })}
                </p>
            </div>
            <button
