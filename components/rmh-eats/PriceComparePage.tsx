...
export default function PriceComparePage() {
    ...
    function handleAdd(result: Result) {
        ...
        toast.success(t("item-added-to-cart", { defaultValue: "{{name}} added to cart!", name: result.item.name }));
    }
    ...
    return (
        <div className="max-w-2xl mx-auto">
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h2 className="text-xl font-bold text-white">{t("price-comparison", { defaultValue: "Price Comparison" })}</h2>
                    <p className="text-sm text-slate-400">{t("price-comparison-subtitle", { defaultValue: "Search a dish to compare prices across all restaurants" })}</p>
                </div>
            </div>
            ...
            <div className="relative mb-5">
                ...
                <input
                    ...
                    placeholder={t("search-placeholder", { defaultValue: 'Try "burger", "chicken", "pasta"...' })}
            ...
            {query.trim().length < 2 && (
                <div className="text-center py-16 text-slate-500">
                    ...
                    <p className="mt-4 font-medium text-slate-400">{t("type-min-chars", { defaultValue: "Type at least 2 characters to search" })}</p>
                    <p className="text-sm mt-1">{t("compare-across-restaurants", { defaultValue: "Compare dishes across all {{count}} restaurants", count: mockRestaurants.length })}</p>
                </div>
            )}
            ...
            {query.trim().length >= 2 && results.length === 0 && (
                <div className="text-center py-16 text-slate-500">
                    <span className="text-5xl">😔</span>
                    <p className="mt-4 font-medium text-slate-400">{t("no-dishes-found", { defaultValue: 'No dishes found for "{{query}}"', query })}</p>
                    <p className="text-sm mt-1">{t("try-different-term", { defaultValue: "Try a different search term" })}</p>
                </div>
            )}
            ...
            {results.length > 0 && (
                <>
                    <p className="text-xs text-slate-500 mb-3">
                        {t("results-count", { defaultValue: "{{count}} result · sorted by price (lowest first)", defaultValue_plural: "{{count}} results · sorted by price (lowest first)", count: results.length })}
                    </p>
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            {isCheapest && (
                                                <span className="rounded-full bg-green-500/20 border border-green-500/30 px-2 py-0.5 text-xs font-medium text-green-400 shrink-0">
                                                    {t("best-value", { defaultValue: "Best Value" })}
                                                </span>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex items-center gap-3 mt-1 text-xs text-slate-500">
                                            ...
                                            {item.vegetarian && <span className="text-green-400">🌿 {t("veg", { defaultValue: "Veg" })}</span>}
                                            {item.spicy && <span className="text-red-400">🌶️ {t("spicy", { defaultValue: "Spicy" })}</span>}
                                        </div>
                                    </div>
