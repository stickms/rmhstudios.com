...
export default function IssueReportPage() {
    ...
    useEffect(() => {
        ...
        const t2 = setTimeout(() => {
            ...
            toast.success(t("refund-approved", { defaultValue: "Refund of ${{amount}} approved!", amount: refundAmt }), { duration: 6000, icon: '💰' });
        }, 8000);
        ...
        return (
            <div className="flex flex-col items-center gap-4 py-24">
                ...
                <button onClick={() => setView('history')} className="text-orange-400 hover:text-orange-300 text-sm">
                    {t("back-to-history", { defaultValue: "Back to history" })}
                </button>
            </div>
        );
    }
    ...
    function handleSubmit() {
        if (!selectedType) {
            toast.error(t("select-issue-type", { defaultValue: "Please select an issue type" }));
            return;
        }
        ...
        setSubmitted(true);
        toast.success(t("issue-reported", { defaultValue: "Issue reported! We're looking into it." }), { icon: '📋' });
    }

    const steps = [
        { key: 'pending', label: t("step-report-received", { defaultValue: "Report Received" }), icon: <CheckCircle2 className="h-4 w-4" /> },
        { key: 'reviewing', label: t("step-under-review", { defaultValue: "Under Review" }), icon: <Search className="h-4 w-4" /> },
        { key: 'resolved', label: t("step-resolved", { defaultValue: "Resolved" }), icon: <CheckCircle2 className="h-4 w-4" /> },
    ];
    ...
    return (
        <div className="max-w-lg mx-auto">
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h2 className="text-xl font-bold text-white">{t("report-an-issue", { defaultValue: "Report an Issue" })}</h2>
                ...
                {!submitted ? (
                    ...
                    >
                        ...
                        <div className="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-4">
                            <h3 className="text-sm font-medium text-slate-400 mb-2">{t("order-summary", { defaultValue: "Order Summary" })}</h3>
                            ...
                            <p className="text-sm font-semibold text-orange-400 mt-2">
                                {t("order-total", { defaultValue: "Total: ${{amount}}", amount: order.total.toFixed(2) })}
                            </p>
                        </div>

                        {/* Issue type */}
                        <div className="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-4">
                            <h3 className="text-sm font-semibold text-white mb-3">{t("what-went-wrong", { defaultValue: "What went wrong?" })}</h3>
                            <div className="space-y-2">
                                {ISSUE_OPTIONS.map((opt) => (
                                    ...
                                    >
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-white">{t(`issue-label-${opt.type}`, { defaultValue: opt.label })}</p>
                                            <p className="text-xs text-slate-400">{t(`issue-desc-${opt.type}`, { defaultValue: opt.desc })}</p>
                                        </div>
                                        {selectedType === opt.type && (
                        ...
                        {/* Description */}
                        <div className="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-4">
                            <h3 className="text-sm font-semibold text-white mb-2">{t("tell-us-more", { defaultValue: "Tell us more (optional)" })}</h3>
                            <textarea
                                value={description}
                                onChange={(e) => setDescription(e.target.value)}
                                placeholder={t("describe-what-happened", { defaultValue: "Describe what happened..." })}
                                rows={3}
                        ...
                        >
                            ...
                            {t("submit-report", { defaultValue: "Submit Report" })}
                        </button>
                    </motion.div>
                ) : (
                    <motion.div
                    ...
                    >
                        <div className="rounded-2xl bg-green-500/10 border border-green-500/30 p-4 flex items-start gap-3">
                            ...
                            <div>
                                <p className="font-semibold text-green-400">{t("report-submitted", { defaultValue: "Report Submitted" })}</p>
                                <p className="text-sm text-slate-400 mt-0.5">
                                    {t("reviewing-issue", { defaultValue: "We're reviewing your issue and will process your refund shortly." })}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-5">
                            <h3 className="font-semibold text-white mb-4">{t("refund-status", { defaultValue: "Refund Status" })}</h3>
                                ...
                                {steps.map((step, i) => {
                                    ...
                                    return (
                                        <div key={step.key} className="flex items-center gap-3">
                                            ...
                                            {isCurrent && refundStep !== 'resolved' && (
                                                <span className="text-xs text-slate-500 ml-auto">{t("processing", { defaultValue: "Processing..." })}</span>
                                            )}
                                            {refundStep === 'resolved' && step.key === 'resolved' && (
                                                <span className="text-xs text-green-400 ml-auto font-medium">{t("approved", { defaultValue: "Approved!" })}</span>
                                            )}
                                        </div>
                                    );
                                })}
                        ...
                        >
                            {t("back-to-order-history", { defaultValue: "Back to Order History" })}
                        </button>
                    </motion.div>
                )}
            </AnimatePresence>
