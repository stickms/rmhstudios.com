...
export function SongDetailsPanel({ song, onPlay, onSongUpdated, readOnly = false }: SongDetailsPanelProps) {
    ...
    const handleCoverSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.size > MAX_COVER_SIZE) {
            toast.error(t("cover-too-large", { defaultValue: "Cover image too large ({{size}} MB). Maximum size is 10 MB.", size: (file.size / 1024 / 1024).toFixed(1) }));
            e.target.value = '';
            return;
        }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            toast.error(t("save-failed", { defaultValue: "Failed to save: {{message}}", message: e.message ?? t("unknown-error", { defaultValue: "Unknown error" }) }));
        } finally {
            setIsSaving(false);
        }
    };
    ...
    return (
        <>
                ...
                <div className="fixed inset-0 z-200 flex items-center justify-center">
                    ...
                    <div className="relative bg-slice-bg rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 flex flex-col gap-4">
                        <div className="flex items-center justify-between">
                            <h3 className="text-lg font-black text-slice-text uppercase tracking-tight">{t("edit-track", { defaultValue: "Edit Track" })}</h3>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slice-text-light uppercase tracking-wider">{t("cover-image", { defaultValue: "Cover Image" })}</label>
                                <label className="flex items-center gap-3 cursor-pointer group">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <span className="text-sm font-bold text-blue-500 group-hover:text-blue-600">{editCoverFile ? editCoverFile.name : t("click-to-change-cover", { defaultValue: "Click to change cover" })}</span>
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slice-text-light uppercase tracking-wider">{t("title-label", { defaultValue: "Title" })}</label>
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slice-text-light uppercase tracking-wider">{t("artist-label", { defaultValue: "Artist" })}</label>
                            ...
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slice-text-light uppercase tracking-wider">{t("bpm-label", { defaultValue: "BPM" })}</label>
                                <Input type="number" value={editBpm} onChange={e => setEditBpm(e.target.value)} className="bg-(--slice-input-bg) text-slice-text border-(--slice-input-border) shadow-[inset_2px_2px_4px_var(--slice-shadow-dark),inset_-2px_-2px_4px_var(--slice-shadow-light)]" placeholder={t("bpm-placeholder", { defaultValue: "e.g. 120" })} />
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-bold text-slice-text-light uppercase tracking-wider">{t("description-label", { defaultValue: "Description" })}</label>
                                <Input value={editDescription} onChange={e => setEditDescription(e.target.value)} className="bg-(--slice-input-bg) text-slice-text border-(--slice-input-border) shadow-[inset_2px_2px_4px_var(--slice-shadow-dark),inset_-2px_-2px_4px_var(--slice-shadow-light)]" placeholder={t("description-placeholder", { defaultValue: "Optional description..." })} />
                            </div>
                        </div>
                        <div className="flex gap-2 pt-1">
                            <Button variant="ghost" className="flex-1 text-slice-text-muted" onClick={() => setShowEdit(false)} disabled={isSaving}>{t("cancel", { defaultValue: "Cancel" })}</Button>
                            <Button className="flex-1 bg-blue-500 hover:bg-blue-600 text-white font-bold gap-2" onClick={handleSave} disabled={isSaving}>
                                ...
                                {isSaving ? t("saving", { defaultValue: "Saving..." }) : t("save", { defaultValue: "Save" })}
                            </Button>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex gap-4 items-start">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <div className="flex flex-col">
                                <span className="text-[10px] font-bold text-slice-text-light uppercase">{t("tempo", { defaultValue: "Tempo" })}</span>
                                <span className="font-mono text-slice-text font-bold text-sm">{Math.round(song.bpm)} BPM</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[10px] font-bold text-slice-text-light uppercase">{t("duration", { defaultValue: "Duration" })}</span>
                            ...
                            <div className="flex flex-col">
                                <span className="text-[10px] font-bold text-slice-text-light uppercase">{t("difficulty", { defaultValue: "Difficulty" })}</span>
                        ...
                        <div className="flex items-center gap-2 mt-3 flex-wrap">
                            <div className="flex items-center gap-1.5 bg-slice-shadow-dark/20 px-2 py-1 rounded-md border border-slice-shadow-dark/30" title={t("total-plays", { defaultValue: "Total plays" })}>
                ...
                {!readOnly && (
                    <div className="mt-4 flex items-center gap-3">
                        ...
                        >
                            ...
                            <span className="uppercase tracking-wide">{t("start-game", { defaultValue: "Start Game" })}</span>
                        </Button>
                        <Button
                            variant="ghost"
                        ...
                        <div className="flex flex-col items-center px-4 py-2 bg-slice-card-bg rounded-lg border border-slice-shadow-dark/50">
                            <div className="text-[10px] font-bold text-slice-text-light uppercase">{t("multiplier", { defaultValue: "Multiplier" })}</div>
            ...
            <div className="p-4 border-b border-slice-shadow-dark/30">
                <h3 className="text-sm font-bold text-slice-text uppercase mb-3">{t("difficulty", { defaultValue: "Difficulty" })}</h3>
                <div className="grid grid-cols-4 gap-1.5 mb-4">
                    {(['easy', 'normal', 'hard', 'expert'] as Difficulty[]).map(d => {
                        ...
                        return (
                            ...
                            >
                                <div>{d}</div>
                                <div className={`text-[10px] font-normal mt-0.5 ${isActive ? 'text-white/80' : 'text-slice-text-muted'}`}>{t("notes-pct", { defaultValue: "{{pct}} notes", pct: noteMap[d] })}</div>
                            </button>
                        );
                    })}
                </div>

                <h3 className="text-sm font-bold text-slice-text uppercase mb-3">{t("game-modifiers", { defaultValue: "Game Modifiers" })}</h3>
                ...
                <div className="space-y-2">
                    <div className="flex justify-between items-center">
                        <h3 className="text-xs font-bold text-slice-text-muted uppercase">{t("playback-speed", { defaultValue: "Playback Speed" })}</h3>
                        <div className="flex items-center gap-2">
                            {modifiers.speed < 1.0 && (
                                <span className="text-[9px] font-bold uppercase tracking-wide bg-orange-500/20 text-orange-400 px-1.5 py-0.5 rounded-full">{t("unranked", { defaultValue: "Unranked" })}</span>
                            )}
            ...
            {/* Leaderboard Section */}
            <div className="p-4 border-b border-slice-shadow-dark/30">
                <h3 className="text-sm font-bold text-slice-text uppercase mb-3">{t("leaderboard", { defaultValue: "Leaderboard" })}</h3>
            ...
            {/* Comments Section */}
            <div className="p-4">
                <h3 className="text-sm font-bold text-slice-text uppercase mb-3">{t("comments", { defaultValue: "Comments" })}</h3>
