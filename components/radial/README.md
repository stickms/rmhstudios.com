...
| `MetaballCursor.tsx` | Site-wide gooey cursor (its own `#rmh-goo` filter + `mix-blend-mode: difference`) that trails and swells over interactive elements. Works on both pointer kinds — trails the mouse on desktop, blooms under the thumb on touch (press → follow → release) — with frame-rate-independent delta-time smoothing (stays smooth at 120/144Hz), off under reduced-motion.                                                                                                                                                                                                                                                                                                                                    |
