...
export default function BreadcrumbTrail({
  ...
  return (
    <div
      ref={scrollRef}
      className="flex items-center gap-1 overflow-x-auto rounded-lg border border-[var(--rmhbox-border)] bg-[var(--rmhbox-surface)] px-3 py-2 scrollbar-thin"
    >
      <span className="mr-1 text-xs font-medium text-[var(--rmhbox-text-muted)] shrink-0">
        Path ({path.length}):
      </span>
      {path.map((title, idx) => {
        ...
        return (
          <span key={`${idx}-${title}`} className="flex items-center gap-1 shrink-0">
            {idx > 0 && (
              <ChevronRight size={10} className="text-[var(--rmhbox-text-muted)]/50" />
            )}
            <button
              ...
              className={`
                ...
                ${isCurrent
                  ? 'bg-[var(--rmhbox-accent)]/20 text-[var(--rmhbox-accent)] font-bold'
                  : isStart
                    ? 'text-green-400 hover:bg-green-500/10'
                    : isTarget
                      ? 'text-yellow-400 hover:bg-yellow-500/10'
                      : 'text-[var(--rmhbox-text-muted)] hover:bg-[var(--rmhbox-surface-hover)] hover:text-[var(--rmhbox-text)]'
                }
