...
export default function RhymeTimeInput({
  ...
  return (
    <div className="flex w-full max-w-4xl gap-6">
      {/* Main panel */}
      <div className="flex flex-1 flex-col items-center gap-6 text-[var(--rmhbox-text)]">
        {/* Timer bar */}
        ...
        <div className="text-center">
          <p className="text-xs uppercase tracking-wider text-[var(--rmhbox-text-muted)]">
            Rhyme with
          </p>
          <h2 className="mt-1 text-5xl font-extrabold text-[var(--rmhbox-accent)]">{rootWord}</h2>
        </div>
        ...
        <div className="flex w-full max-w-md gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-[var(--rmhbox-border)] bg-[var(--rmhbox-surface)] px-4 py-2 text-sm text-[var(--rmhbox-text)] placeholder:text-[var(--rmhbox-text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--rmhbox-accent)] disabled:opacity-50"
          />
          <button
            onClick={handleSubmit}
            disabled={atLimit || !value.trim()}
            className="flex items-center gap-2 rounded-lg bg-[var(--rmhbox-accent)] px-4 py-2 text-sm font-medium text-white transition-opacity hover:opacity-90 disabled:opacity-40"
          >
      ...
      <aside className="hidden w-52 shrink-0 rounded-xl border border-[var(--rmhbox-border)] bg-[var(--rmhbox-surface)] p-4 md:block">
        <h3 className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-[var(--rmhbox-text-muted)]">
        ...
        <ul className="space-y-1.5">
          {submissionCounts
            ...
            .map((p) => (
              <li key={p.userId} className="flex items-center justify-between text-sm text-[var(--rmhbox-text)]">
                <span className="truncate">{p.userName}</span>
                <span className="font-mono text-[var(--rmhbox-text-muted)]">{p.count}</span>
              </li>
            ))}
        </ul>
      </aside>
