  ...
  return (
    <div className="flex flex-col items-center gap-4 p-4 w-full max-w-md mx-auto">
      ...
      <div className="text-sm text-(--app-text-muted)">
        {t("producer-label", { defaultValue: "Producer:" })} <span className="font-semibold text-(--app-text)">{producerName}</span>
      ...
      <div className="w-full flex flex-col gap-1">
        ...
        {sorted.map((r) => (
          <div
            key={r.userId}
            className="flex items-center justify-between px-3 py-2 rounded-lg bg-(--app-surface) text-sm"
          >
            <div className="flex items-center gap-2">
              <span>{r.guessedCorrectly ? '✅' : '❌'}</span>
              <span className="text-(--app-text)">{r.userName}</span>
              {r.guessNumber != null && (
            ...
            <span className="font-semibold text-(--app-accent)">{r.points} {t("pts", { defaultValue: "pts" })}</span>
          </div>
        ))}
      </div>
    </div>
