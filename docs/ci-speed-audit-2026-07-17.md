...
1. **Parallelize the deploy pipeline** — the CI gate runs *serially before*
   the image build today (~4 min of pure latency), and the full image build
   serially *after* the slim one (~1.5–2.5 min tail for seconds of real
   ...
   vs Go parallelism *inside* the build is already handled — BuildKit runs
...
2. **Fix the permanently-red `i18n-catalogs` test** — every `web-ci` run on
   ...
   *never runs on PRs*. Build breakage is discovered only during the
...
3. **Consolidate the PR workflow fan-out** — a typical frontend PR fires
   ...
   browser boot on every PR**. The GitHub Free-plan cap is 20 *concurrent jobs
   account-wide*, so a single push can saturate the pool and everything queues.
...
- **Every single `web-ci` run in the last two days is red** — all fail at the
  ...
  audit` and `Build` steps are therefore *skipped on every PR*.
...
can't ship — but nothing about that requires *serialization*. The thing that
...
job run the *same* install/typecheck/lint/test on the same commit at the same
...
1. **`RUN --mount=type=cache` mounts do not persist on hosted runners.** The
   ...
   restores *layers*, never mount contents — and each hosted runner starts
...
3. **The weekly `docker-build.yml` scan builds on amd64** (no `platforms:`)
   with a *different* cache scope, so it neither warms nor reuses the deploy's
...
- Several *report-only* workflows pay a full install just to print a summary:
...
- GitHub Free plan allows **20 concurrent jobs account-wide**. A single PR
  push can occupy the entire pool, so *wall-clock to all-green* is set by
...
### R1 — Restructure `deploy.yml`: gate the *trigger*, not the *build* (saves ~4 min/deploy)
...
- **Within** the slim-image build, BuildKit already parallelizes the frontend
  ...
  The Go services are *not* what makes the build slow — Vite is.
...
across two *separate* invocations (per the Dockerfile's own comment). A
...
*and* deletes the WEB_IMAGE plumbing from the workflow. Combined with R1, the
...
*web* image, not the full one). Treat as optional follow-up, not phase 1.
...
2. If more is needed after (1), persist the two hot `RUN --mount=type=cache`
   ...
   Vite build *incremental* in CI rather than cold. Measure before/after; the
...
- Set the `BAZEL_REMOTE_CACHE` secret (BuildBuddy free tier or a small
  ...
  cache.
- Gate `senior-review`'s job on the author condition *before* spinning the
...
- **Build once, promote the artifact.** The SHA-tagged image *is* the
  artifact; CI green *promotes* it (tag/`:latest` + webhook) rather than
