...
**This is a decision to make, not just a cleanup.** The classes are gone, so the
source no longer claims motion it does not have — but a dialog that scales in is
better than one that pops, and the design language asks for exactly that
(§7: `scaleIn`, `popIn`, `overlay`, `modalContent` in `lib/motion.ts`). The two
ways to get it back:

1. **Adopt `lib/motion.ts`** on those surfaces — the documented system, no new
   dependency, framer-motion is already in the bundle. More work, and the right
   answer.
2. **Add `tw-animate-css`** — the maintained v4 successor, one `@import`,
   restores all 103 in a line. Fast, but it is a *fourth* animation vocabulary
   next to framer-motion, the CSS enters, and the radial layer.

Not done here because it is a visible change across core chrome and this pass
has not been looked at in a browser. Rule 9 in the design gate now fails on the
dead vocabulary, so the choice cannot be deferred by accident a second time.
