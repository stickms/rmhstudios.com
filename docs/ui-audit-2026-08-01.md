...
### UIP-7 [P3 · flagged] `transition-all`, 46 site-tier call sites

`transition: all` makes the browser watch every animatable property on the
element, including layout ones. The practical cost is modest (engines only
animate what actually changes) and converting all 46 risks silently dropping an
intended transition, so this is a cleanup for a pass that can verify visually,
...
`lib/__tests__/design-consistency.test.ts` grew three rules, all site-tier only,
