...
### UIP-2 [P1 · fixed] Inert plumbing left behind
...
primitives — the selector that read them is gone. Removed, along with the dead
`.hover-lift` CSS (zero call sites, and it carried a standing `will-change:
transform`, which is a promoted layer per element for an effect nobody uses).
...
### UIP-4 [P1 · flagged, deliberately not "fixed"] The GL liquid layer is never initialised

`lib/liquid-gl/` is a complete WebGPU/WebGL2 renderer — ~60KB of source across
`index.ts`, both renderers, `scene.ts`, `trust.ts`, `detect.ts` — and
**`initLiquidGL()` has no caller anywhere in the repo.** `index.ts`'s own
docblock says "dynamic-imported from Providers after first paint"; Providers does
not. So `setLiquidActive(true)` never runs, `isLiquidActive()` is permanently
false, `html.liquid-gl` is never set, and:

- the scene light `useGlassLight` published every pointer frame had **no reader**;
- `setAuroraOffset`'s mirror has no reader (`getAuroraX/Y` are called by nothing);
- `useLiquidBody` — used by `liquid-morph` and `liquid-pop` — always reports
  inactive, so those always take their CSS/SVG fallback path.

This is reported rather than repaired. Wiring it up would light a full-screen
WebGL canvas on every page, which is the opposite of a 120fps goal, and it is a
product decision about whether the shader tier ships at all. What this pass did
do is stop paying for it: the pointer work that fed it is gone.

**If the intent is that it stays parked,** the honest move is to delete the
renderer and the `scene-light` setters and keep the CSS/SVG tier as the only
tier. **If the intent is that it ships,** it needs the Providers wire plus a
measured pass of its own.

### UIP-5 [P2 · flagged] `.glass-liquid` repaints on a `background-position` keyframe

`animation: glass-sheen 9s ease-in-out infinite` animates `background-position`,
which is a paint property — so a `.glass-liquid` surface repaints continuously,
forever, and at 120Hz that is 120 repaints/second of a panel. Mitigating facts:
the modifier is rationed to ≤3 per page, only four files use it, `perf-lite`
stops it, and the keyframe's long dwells mean the value is static for ~62% of
each cycle.

The compositor-correct form is a transform-translated child layer on `::after`
(which this pass freed, UIP-1) clipped by the parent. It is **not** done here
because the clip that requires would also clip the two surfaces that compose
`.glass-liquid` with `.glass-refract--prism` and an overflowing featured badge,
and that needs a browser to check rather than a static read.
