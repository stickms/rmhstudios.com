...
| `level-schema.test.ts`              | Every shipped level parses; contrast ≥ 7; spawns ≥ minPlayers; goal in bounds; no prop/spawn overlap; the manifest and the world directories agree in both directions; each world's §6.7 solo/co-op split |
| `level-reachability.test.ts`        | **A route exists from spawn to goal.** Added after `w4-03` shipped through every other gate with its goal stranded 700px out over a void — the author's notes described a "goal-side pier" that was never placed. A graph of grabbable things with one-swing edges; models that falling is free, that a sized prop is a surface, and that a launcher is a teleport. See §21 risk 10 for what it deliberately does not model |
...
| 11 | **The swing was partly powered by a bug, and fixing the bug cost reach.** `correctPosition` added the whole of every joint correction to velocity, each step, per joint — an unbounded energy pump. It drove a resting character through the floor and out of the world (y=11571 measured) and spun the arms on their own; `ARM_FORCE_MAX` had been retuned *down* to 0.0011 to compensate, so the two were propping each other up. With the pump gone, a two-player chain crosses **194px where feel test 2 asks for 420**. | The pump is fixed and the test is **skipped, loudly**, with the whole story in its comment — a known regression beats a silent one, and lowering `GAP` would have made it silent. Restoring the reach is a design decision: a release impulse, a longer arm, or accepting a shorter two-player span and re-authoring the levels that assume otherwise. Measured dead ends so the next person skips them: raising `ARM_FORCE_MAX` to 0.0034 brings the flying back without restoring reach (183px), and raising a gripped arm's authority makes it *worse* (194 → 118 at 4×) because the chain collapses instead of extending. |
| 10 | **A level can pass every gate and be unplayable.** Three of World 4's nine levels shipped with their goal stranded past an unbridged gap — `w4-03` by 700px over a void, `w4-04` by 500px, `w4-06` by 300px with no ground beneath it. All three passed schema, contrast, spawn/goal-in-bounds, overlap, signal-producer, body-budget and checkpoint-spacing checks, because none of those asks the only question that matters: can you finish it. Two of the three were *described* correctly in `authorNotes` ("the checkpoint sits on the goal-side pier") and simply never built. | `level-reachability.test.ts` now gates both halves — a one-swing graph from spawn to goal, and solid ground under the goal. Note the two are independent: `w4-06` was inside one swing and so passed the reachability half, and was caught only by the grounding half. The model deliberately does not simulate swing arcs, doors opening on signals, or carried crates; it is a floor that catches the empty void, not a proof of completability. **Widen the model rather than deleting a case** when a legitimate route trips it. |
