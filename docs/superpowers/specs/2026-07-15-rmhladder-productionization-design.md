...
- Adapters' discovery return an explicit **fetch-success** indicator distinct from "zero jobs found," so recheck can tell _"board fetched OK and is genuinely empty"_ (→ jobs should strike/expire) from _"fetch failed/ambiguous"_ (→ skip, no strikes). This removes the `boardJobs.length > 0` heuristic in `recheck.ts`.
...
- Distinguish _worker-down_ (no run rows appearing) from _runs-failing_ (rows with high errors).
...
- Alerting fan-out: reuse the user-facing alert channels for _operator_ alerts, or a lighter in-app/health-only surface? (Default: start with health surface + existing notification path; expand only if cheap.)
