...
| SEC-06  | Remediated in configuration              | Compose/Kubernetes add non-root enforcement, read-only roots, dropped capabilities, no-new-privileges, PID limits, seccomp, tmpfs, and isolated repository/worktree volumes. Runtime smoke testing requires Docker/Helm, which were unavailable on the audit workstation.                                                       |
