...
//   node build.mjs
//
// Output: docs/textbook/indonesia-a-comprehensive-history.(html|pdf)
...
// Replace <img ... src="img/NAME" ...> with an inlined base64 data-URI read from
// assets/img/NAME, so the output PDF is fully self-contained/offline. Throws if a
// referenced image file is missing (fail loud rather than ship a broken figure).
const IMG_DIR = resolve(ASSETS, "img");
...
manifest.sections.forEach((sec) => {
  ...
  const frag = inlineImages(readFileSync(file, "utf8"));
...
const outHtml = resolve(ROOT, "indonesia-a-comprehensive-history.html");
const outPdf = resolve(ROOT, "indonesia-a-comprehensive-history.pdf");
