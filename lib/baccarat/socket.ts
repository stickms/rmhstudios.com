...
import { useBaccaratStore } from './store';
import { S2C } from './events';

let socket: Socket | null = null;

export async function connectToBaccarat(): Promise<Socket> {
  ...
  const store = useBaccaratStore.getState();
  ...
  socket.on(S2C.CARD_REVEAL, (data) => store.handleCardReveal(data));
...
export function getBaccaratSocket(): Socket | null {
  return socket;
}

export function disconnectFromBaccarat(): void {
  if (socket) {
    socket.removeAllListeners();
    socket.disconnect();
    socket = null;
  }
  useBaccaratStore.getState().reset();
}

const balanceUpdateCallbacks = new Set<(coins: number) => void>();

export function onBaccaratBalanceUpdate(cb: (coins: number) => void): () => void {
