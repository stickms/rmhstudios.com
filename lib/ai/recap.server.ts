...
export async function generateWeeklyRecap(userId: string): Promise<WeeklyRecap> {
  ...
  const [posts, likesReceived, commentsReceived, newFollowers, achievements, topPost, streak] =
    await Promise.all([
      ...
      prisma.rMHarkComment.count({
        where: { rmhark: { userId }, createdAt: { gte: start }, NOT: { userId } },
      }),
    ...
    try {
      const res = await deepseek.chat.completions.create({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `Write a recap from these weekly stats: ${JSON.stringify(stats)}`,
          },
        ],
        max_tokens: 160,
        temperature: 0.7,
