...
const AUDIO_SIGNATURES: { name: string; check: (buf: Buffer) => boolean }[] = [
  ...
  {
    name: 'MP3 (ID3)',
    check: (buf) => buf.length >= 3 && buf[0] === 0x49 && buf[1] === 0x44 && buf[2] === 0x33,
  },
...
export function validateAudioBuffer(buffer: Buffer): { ok: true } | { ok: false; error: string } {
  if (buffer.length > AUDIO_MAX_BYTES) {
    return {
      ok: false,
      error: `Audio file too large. Maximum size is ${AUDIO_MAX_BYTES / 1024 / 1024} MB.`,
    };
  }
  ...
  if (!valid) {
    return {
      ok: false,
      error:
        'Invalid audio file. Only MP3, WAV, OGG, and FLAC are allowed. File content did not match a supported format.',
    };
  }
  return { ok: true };
}
...
export function validateImageBuffer(buffer: Buffer): { ok: true } | { ok: false; error: string } {
  if (buffer.length > COVER_MAX_BYTES) {
    return {
      ok: false,
      error: `Cover image too large. Maximum size is ${COVER_MAX_BYTES / 1024 / 1024} MB.`,
    };
  }
...
export function detectImageExt(buffer: Buffer): '.png' | '.jpg' | '.webp' | '.gif' | null {
  if (
    buffer.length >= 8 &&
    buffer[0] === 0x89 &&
    buffer[1] === 0x50 &&
