...
const CSV_COLUMNS: ReadonlyArray<{ header: string; read: (day: WatchDayDTO) => string | number }> = [
  ...
  { header: 'job_mentions', read: (day) => day.jobMentions },
  { header: 'typing_starts', read: (day) => day.typingStarts },
  { header: 'typing_abandoned', read: (day) => day.typingAbandoned },
  { header: 'typing_abandoned_sec', read: (day) => day.typingAbandonedSec },
...
export function toDossierJson(state: WatchStateDTO): DossierExport {
  return {
    ...
    notes: [
      ...
      'job_mentions counts messages matching a job-hunting phrase list, decided when the message arrived. It is a keyword match, not a judgement.',
      'typing_abandoned counts compose sessions that produced no message. Only settled sessions are counted; one still being typed is not an abandonment.',
    ],
    totals: state.totals,
    days: state.days,
    summaries: {
