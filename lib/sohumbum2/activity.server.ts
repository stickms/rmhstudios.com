/**
 * Reads the `discord_watch_*` tables into the shapes `/sohumbum2` renders.
 *
...
export async function getLive(now: Date = new Date()): Promise<WatchLiveDTO> {
  const [live, voice, playing, lastMessage] = await Promise.all([
    ...
    prisma.discordWatchPresenceSession.findFirst({
      where: { discordId: SUBJECT_DISCORD_ID, endedAt: null },
      orderBy: { startedAt: 'desc' },
      select: { activityName: true, details: true, state: true, startedAt: true },
    }),
  ...
  return {
    ...
    playing: playing
      ? {
          name: playing.activityName,
          details: playing.details,
          state: playing.state,
          startedAt: playing.startedAt.toISOString(),
          durationSec: secondsBetween(playing.startedAt, now),
        }
      : null,
