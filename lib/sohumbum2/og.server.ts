/**
 * What `/sohumbum2`'s Open Graph cards and meta descriptions say.
 *
...
export async function buildDayCard(dateKey: string): Promise<PageCardData | null> {
  ...
  return {
    ...
    cacheKey: `sohumbum2:day:${dateKey}:${snapshot.updatedAt ?? 'empty'}`,
...
export async function buildOverviewCard(): Promise<PageCardData> {
  ...
  return {
    ...
    cacheKey: `sohumbum2:overview:${state.generatedAt.slice(0, 16)}:${live.voice ? 'live' : 'idle'}`,
