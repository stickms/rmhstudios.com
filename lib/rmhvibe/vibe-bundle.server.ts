...
const ESM_REACT_VERSION = '19';

/**
 * Whether the pre-built `public/vibe-packages/` bundles actually exist on disk.
 *
 * Those bundles are generated by `pnpm build-vibe-packages` (part of `pnpm build`)
 * and are gitignored, so a dev server — or any deploy where that step didn't run —
 * won't have them. Previously the importmap pointed React (and every hosted lib)
 * at `/vibe-packages/*.js` unconditionally: when the files were missing, EVERY
 * generated page failed to import React and rendered a blank "Unknown error"
 * screen, with no fallback because React was hosted-only.
 *
 * We check a few known locations once (dev `public/`, and the prod build outputs)
 * and cache the result. When present we self-host (fast, no third-party CDN); when
...
(function(){
  ...
  window.addEventListener('error', function(ev){
    // Always log the raw event so the full detail is in the console even when the
    // on-page card can only show a sanitized message.
    try { console.error('[vibe] error event:', ev, ev && ev.error); } catch(_){}
    ...
    var e = ev && ev.error;
    // Where it happened — for a module-evaluation throw this is the bundle URL/line.
    var where = '';
    if (ev && ev.filename) {
      where = '\\n\\nat ' + ev.filename + (ev.lineno ? ':' + ev.lineno : '') + (ev.colno ? ':' + ev.colno : '');
    }
    var detail = e ? (e.stack || (e.name + ': ' + e.message)) : (ev && ev.message) || '';
    // An opaque-origin iframe sanitizes cross-origin script errors to an empty
    // message with no stack — the classic "Unknown error". Say so, and point at the
    // two real culprits (a failed dependency, or the console/Network tab) instead.
    if (!detail) {
      detail = 'A script error occurred but the browser withheld its details (this page runs sandboxed on an opaque origin). The usual cause is a dependency that failed to load \\u2014 open the browser console and the Network tab and look for a failed request (e.g. React or another package).';
    }
    show('This vibe hit a runtime error', detail + where);
  }, true);
...
function buildImportMap(
  ...
  const hosted = hostedPackagesAvailable();
  const hostedUrl = (stem: string) => `${VIBE_PACKAGES_BASE}/${stem}.js?r=${VIBE_PACKAGES_REVISION}`;

  // esm.sh URL for a hosted package when self-hosting is unavailable. React and
  // react-dom are pinned and kept on a SINGLE instance: react-dom (and every other
  // lib) carries `?external=react,react-dom` so it imports the bare `react` the
  // importmap already resolves, rather than bundling a second copy.
  const esmHostedUrl = (name: string, subpath?: string): string => {
    if (name === 'react' || name === 'react-dom') {
      const base = `https://esm.sh/${name}@${ESM_REACT_VERSION}`;
      const ext = name === 'react-dom' ? '?external=react' : '';
      return subpath ? `${base}/${subpath}${ext}` : `${base}${ext}`;
    }
    const version = deps[name];
    const spec = version ? `${name}@${version}` : name;
    const url = subpath ? `https://esm.sh/${spec}/${subpath}` : `https://esm.sh/${spec}`;
    return `${url}?external=react,react-dom`;
  };

  for (const [name, entry] of Object.entries(VIBE_PACKAGES)) {
    if (entry.tier !== 'hosted') continue;
    imports[name] = hosted ? hostedUrl(hostedStem(name)) : esmHostedUrl(name);
    for (const sub of entry.subpaths ?? []) {
      imports[`${name}/${sub}`] = hosted ? hostedUrl(hostedStem(name, sub)) : esmHostedUrl(name, sub);
    }
