...
describe('private resume ownership service', () => {
  it('keeps the DB ownership row when private object deletion fails', async () => {
    ...
    const storage = {
      put: vi.fn(),
      get: vi.fn(),
      delete: vi.fn().mockRejectedValue(new Error('S3 unavailable')),
    };
    await expect(deleteResume(prisma, 'user-1', 'resume-1', storage)).rejects.toThrow(
      'S3 unavailable',
    );
  ...
  it('refuses to read a storage key outside the authenticated owner namespace', async () => {
    ...
    await expect(downloadResume(prisma, 'user-1', 'resume-1', storage)).rejects.toThrow(
      'not found',
    );
