...
function encryptionKey(secret = configuredSecret()): Buffer {
  ...
  if (
    base64.length === 32 &&
    base64.toString('base64').replace(/=+$/, '') === secret.replace(/=+$/, '')
  )
    return base64;
  return createHash('sha256').update(secret, 'utf8').digest();
}

export function encryptResumeText(
  plaintext: string,
  opts: { secret?: string; aad?: string } = {},
): string {
...
export function decryptResumeText(
  payload: string,
  opts: { secret?: string; aad?: string } = {},
): string {
  const [version, ivRaw, tagRaw, encryptedRaw, ...extra] = payload.split(':');
  if (version !== VERSION || !ivRaw || !tagRaw || encryptedRaw === undefined || extra.length)
    throw new Error('Invalid encrypted resume payload');
  const decipher = createDecipheriv(
    'aes-256-gcm',
    encryptionKey(opts.secret),
...
export function encryptResumeFile(
  plaintext: Buffer,
  opts: { secret?: string; aad?: string } = {},
): Buffer {
  const iv = randomBytes(12);
...
export function decryptResumeFile(
  payload: Buffer,
  opts: { secret?: string; aad?: string } = {},
): Buffer {
  if (
    payload.length < FILE_MAGIC.length + 28 ||
    !payload.subarray(0, FILE_MAGIC.length).equals(FILE_MAGIC)
  ) {
