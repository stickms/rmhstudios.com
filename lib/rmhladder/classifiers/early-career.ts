...
const POSITIVE: Array<[RegExp, number]> = [
  [/\bsummer analyst\b/i, 40],
  [/\bsummer associate\b/i, 40],
  [/\bintern(ship)?\b/i, 35],
  [/\bnew grad(uate)?\b/i, 35],
  [/\brotational (analyst )?program\b/i, 35],
  [/\bleadership development program\b/i, 35],
  [/\banalyst (development )?program\b/i, 30],
  [/\bearly careers?\b/i, 30],
  [/\bcampus\b/i, 20],
  [/\buniversity program\b/i, 25],
  [/\bentry[- ]level\b/i, 25],
  [/\brecent graduates?\b/i, 25],
  [/\bstudents?\b/i, 15],
  [/\bassociate consultant\b/i, 25],
  [/\bbusiness analyst\b/i, 15],
  [/\b(sophomore|junior|senior year|freshman)\b/i, 20],
  [/\bmba intern\b/i, 30],
  [/\bclass of 20(2[5-9]|3[0-2])\b/i, 25],
  [/\b20(2[5-9]|3[0-2])\b/, 10],
];
const NEGATIVE: Array<[RegExp, number]> = [
  [/\bvice president\b|\bvp\b/i, 60],
  [/\b(executive|managing) director\b/i, 60],
  [/\bdirector\b/i, 50],
  [/\bprincipal\b/i, 50],
  [/\bhead of\b/i, 45],
  [/\bsenior\b(?! year)/i, 30],
  [/\bstaff (engineer|scientist)\b/i, 40],
  [/\blead\b/i, 25],
  [/\bmanager\b/i, 25],
  [/\bexpert\b/i, 25],
  [/\bexperienced professional\b/i, 40],
  [/\blateral\b/i, 30],
  [/\b([5-9]|1[0-9])\+? ?(years|yrs)\b/i, 45],
];
...
export function classifyEarlyCareer(title: string, description = ''): EarlyCareerResult {
  ...
  const strongTitle =
    /\b(summer analyst|summer associate|new grad(uate)?|early careers?|intern(ship)?|rotational)\b/i;
  ...
  const hardNeg =
    /\b(vice president|managing director|executive director|senior(?! year)|director|principal|manager|head of|staff engineer)\b/i;
  const hardPos =
    /\b(summer analyst|summer associate|intern(ship)?|new grad|early careers?|rotational|campus)\b/i;
