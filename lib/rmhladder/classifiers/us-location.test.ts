...
describe('classifyUSLocation', () => {
  it('explicit country wins', () => {
    ...
    expect(classifyUSLocation({ country: 'United Kingdom', locationRaw: 'London' }).isUS).toBe(
      false,
    );
  });
  ...
  it('detects Remote US', () => {
    ...
    expect(classifyUSLocation({ locationRaw: 'Hybrid - New York, NY' }).remoteStatus).toBe(
      'hybrid',
    );
  });
  it('full state names count', () => {
    expect(classifyUSLocation({ locationRaw: 'Austin, Texas' })).toMatchObject({
      isUS: true,
      state: 'TX',
    });
  });
  ...
  it('New Mexico is US despite the mexico country hint', () => {
    expect(classifyUSLocation({ locationRaw: 'Albuquerque, New Mexico' })).toMatchObject({
      isUS: true,
      state: 'NM',
    });
