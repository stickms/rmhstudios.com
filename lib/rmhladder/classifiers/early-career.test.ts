...
describe('classifyEarlyCareer', () => {
  ...
  it('experience requirements in description push to no', () => {
    const r = classifyEarlyCareer(
      'Strategy Associate',
      'Requires 7+ years of consulting experience.',
    );
    expect(r.classification).toBe('no');
  });
  it('program types are detected in priority order', () => {
    expect(classifyEarlyCareer('2026 Rotational Analyst Program').programType).toBe(
      'rotational_program',
    );
    expect(classifyEarlyCareer('Finance Leadership Development Program').programType).toBe(
