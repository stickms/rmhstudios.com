...
const NON_US_COUNTRY_HINT =
  /\b(kingdom|canada|india|singapore|australia|germany|france|japan|china|ireland|switzerland|(?<!new )mexico|brazil|poland|hong ?kong)\b/i;
...
const NON_US_CITIES =
  /\b(london|toronto|vancouver|montreal|paris|frankfurt|dublin|zurich|geneva|mumbai|bangalore|bengaluru|singapore|sydney|tokyo|hong ?kong|shanghai|beijing|warsaw|madrid|milan|amsterdam|tel aviv|mexico city|s[aã]o paulo)\b/i;
...
export function classifyUSLocation(input: LocationInput): LocationResult {
  ...
  if (remoteStatus === 'remote_us') {
    if (/\b(us|usa|united states)\b/i.test(lower) || !raw.replace(/remote|[-()]/gi, '').trim()) {
      return {
        isUS: /\b(us|usa|united states)\b/i.test(lower) ? true : null,
