...
const APPLY_MARKERS =
  /apply now|apply for this|submit (your )?application|start application|\bapply\b/i;
const REQ_ID_MARKERS =
  /job id|requisition|req(?:uisition)? ?(id|number|#)|posting number|R-\d{3,}|REQ-?\d{3,}|\b\d{4}-[A-Z]{2,}-\d{2,}\b/i;
...
export async function verifyGenericUrl(args: {
  ...
  if (res.status === 403 || res.status === 429)
    return { ...base, httpStatus: res.status, blocked: true };
  ...
  const tokens = normalizeTitle(args.jobTitle)
    .split(' ')
    .filter((t) => t.length > 1);
  const hitCount = tokens.filter((t) => lower.includes(t)).length;
  const titleMatch = tokens.length > 0 && hitCount / tokens.length >= 0.6;

  const jobLinkCount = root
    .querySelectorAll('a')
    .filter((a) => /job|career|posting|position/i.test(a.getAttribute('href') ?? '')).length;
  const isSearchResultsPage =
    SEARCH_MARKERS.test(text) ||
    (jobLinkCount >= 10 && !titleMatch) ||
    (jobLinkCount >= 10 && /[?&]q=/.test(args.url));
