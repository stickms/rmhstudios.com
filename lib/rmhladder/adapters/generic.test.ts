...
describe('verifyGenericUrl', () => {
  const args = {
    ...
    jobTitle: 'Investment Banking Summer Analyst 2027',
  };
  ...
  it('detects closed language', async () => {
    const e = await verifyGenericUrl({
      ...args,
      fetchImpl: page('<p>This position is no longer accepting applications.</p>'),
    });
  ...
  it('search results page detected', async () => {
    const links = Array.from(
      { length: 15 },
      (_, i) => `<a href="/careers/job/${i}">Job ${i}</a>`,
    ).join('');
    const e = await verifyGenericUrl({
      ...args,
      url: 'https://example.com/careers/search?q=analyst',
      fetchImpl: page(`<html><body><h2>Search Results</h2>${links}</body></html>`),
    });
  ...
  it('partial title match still counts (60% token overlap)', async () => {
    const e = await verifyGenericUrl({
      ...args,
      fetchImpl: page('<h1>Investment Banking Summer Analyst</h1><p>Goldman Sachs</p><a>apply</a>'),
    });
  ...
  it('data: URL rejected before fetch; fetchImpl never called', async () => {
    ...
    const e = await verifyGenericUrl({
      ...
      fetchImpl: trackingFetch as typeof fetch,
    });
