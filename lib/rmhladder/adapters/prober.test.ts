...
describe('candidateSlugs', () => {
  it('generates ordered deduped candidates', () => {
    expect(candidateSlugs('JPMorgan Chase')).toEqual([
      'jpmorganchase',
      'jpmorgan-chase',
      'jpmorgan',
    ]);
...
describe('probeSlug', () => {
  ...
  it('lever bare-array shape', async () => {
    expect(await probeSlug('lever', 'plaid', stub(200, '[{},{},{}]'))).toEqual({
      live: true,
      jobCount: 3,
    });
  });
  it('smartrecruiters probes the company endpoint, not postings', async () => {
    ...
    expect(await probeSlug('smartrecruiters', 'honeywell', company)).toEqual({
      live: true,
      jobCount: 0,
    });
    ...
    expect(await probeSlug('smartrecruiters', 'zzznotreal', catchAll)).toEqual({
      live: false,
      jobCount: 0,
    });
    const notFound = stub(404, '');
    expect(await probeSlug('smartrecruiters', 'zzznotreal', notFound)).toEqual({
      live: false,
      jobCount: 0,
    });
  });
  it('404 and wrong-shape are dead', async () => {
    expect(await probeSlug('greenhouse', 'nope', stub(404, ''))).toEqual({
      live: false,
      jobCount: 0,
    });
    expect(await probeSlug('greenhouse', 'weird', stub(200, '{"error":"x"}'))).toEqual({
      live: false,
      jobCount: 0,
    });
    expect(await probeSlug('ashby', 'html', stub(200, '<html>'))).toEqual({
      live: false,
      jobCount: 0,
    });
  });
