...
describe('scoreCandidateForJob', () => {
  it('scores deterministically and normalizes common skill aliases', () => {
    ...
    expect(first.matchedSkills).toEqual(
      expect.arrayContaining(['JavaScript', 'React', 'PostgreSQL', 'Amazon Web Services']),
    );
  ...
  it('uses lower confidence when key profile data is absent', () => {
    const result = scoreCandidateForJob(
      { ...candidate, skills: [], yearsExperience: null, education: [], workHistory: [] },
      { ...job, requiredSkills: [], preferredSkills: [], minYearsExperience: null, locations: [] },
    );
  ...
  it('reserves ten points for existing user preferences', () => {
    const preferred = scoreCandidateForJob(candidate, job, {
      ...
      preferredProgramTypes: ['internship'],
      boostKeywords: ['software'],
    });
    const mismatched = scoreCandidateForJob(candidate, job, {
      preferredCities: ['Chicago'],
      preferredProgramTypes: ['mba'],
      boostKeywords: ['accounting'],
    });
