...
const REQUIRED_CONTEXT =
  /\b(required|must have|minimum qualifications?|what you need|basic qualifications?)\b/i;
...
export function profileJobDeterministically(job: LadderJobForProfile): JobProfile {
  ...
  return jobProfileSchema.parse({
    ...
    maxYearsExperience:
      years.map((value) => value.max).find((value): value is number => value !== null) ?? null,
    ...
    earlyCareer:
      job.earlyCareerClassification === 'yes' || job.earlyCareerClassification === 'probable',
  });
}
