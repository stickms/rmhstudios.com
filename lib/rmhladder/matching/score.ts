...
function educationScore(candidate: CandidateProfile, job: JobProfile): number {
  ...
  const candidateText = normalizePhrase(
    candidate.education.map((entry) => `${entry.degree ?? ''} ${entry.field ?? ''}`).join(' '),
  );
  return job.educationLevels.some((level) => candidateText.includes(normalizePhrase(level)))
    ? 10
    : 3;
}
...
function locationScore(candidate: CandidateProfile, job: JobProfile): number {
  ...
  return job.locations.some((location) => candidateLocations.has(normalizePhrase(location)))
    ? 10
    : 1;
}
...
function preferencesScore(job: JobProfile, prefs: MatchUserPreferences): number {
  const hasPrefs = Boolean(
    ...
    prefs.preferredProgramTypes?.length ||
    prefs.boostKeywords?.length,
  );
  ...
  if (prefs.preferredProgramTypes?.length) {
    ...
    if (
      job.programType &&
      prefs.preferredProgramTypes.map(normalizePhrase).includes(normalizePhrase(job.programType))
    )
      matched += 1;
  }
  if (prefs.boostKeywords?.length) {
    ...
    if (prefs.boostKeywords.some((keyword) => haystack.includes(normalizePhrase(keyword))))
      matched += 1;
  }
...
export function scoreCandidateForJob(
  ...
  const matchedSkills = [
    ...new Set([...matchedRequired, ...matchedPreferred].map(([, label]) => label)),
  ];
  const missingSkills = missingRequired.map(([, label]) => label);

  const requiredCoverage =
    requiredSkills.size === 0 ? 1 : matchedRequired.length / requiredSkills.size;
  const preferredCoverage =
    preferredSkills.size === 0 ? 1 : matchedPreferred.length / preferredSkills.size;
  ...
  const rawScore = Math.max(
    0,
    Math.min(100, skills + roleFamily + experience + education + location + preferenceScore),
  );
  ...
  const score =
    requiredSkills.size > 0 && requiredCoverage === 0
