...
describe('RMHLadder alert schedule', () => {
  ...
  it('sends daily and weekly digests only in the local morning slot', () => {
    ...
    expect(
      isDigestDue({ digestFrequency: 'daily', timezone: 'America/New_York' }, mondayMorning),
    ).toBe(true);
    expect(
      isDigestDue({ digestFrequency: 'weekly', timezone: 'America/New_York' }, mondayMorning),
    ).toBe(true);
    expect(
      isDigestDue(
        { digestFrequency: 'weekly', timezone: 'America/New_York' },
