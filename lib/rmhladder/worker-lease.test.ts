...
describe('worker lease', () => {
  it('allows one owner and rejects a second owner before expiry', async () => {
    ...
    await expect(acquireWorkerLease(prisma, { ...lease, ownerId: 'worker-b' }, now)).resolves.toBe(
      false,
    );
  });
  ...
  it('heartbeats and releases only for the current owner', async () => {
    ...
    expect(leases.get(lease.name)?.expiresAt).toEqual(
      new Date(heartbeatAt.getTime() + lease.ttlMs),
    );
    await expect(releaseWorkerLease(prisma, { ...lease, ownerId: 'worker-b' })).resolves.toBe(
      false,
    );
