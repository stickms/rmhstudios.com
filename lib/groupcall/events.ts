 ...
 * inbound events, but note what it is NOT: `check()` returns `true` for a
 * name it does not know (`server/shared/rate-limit.ts`), so an unlisted event
 * is *unmetered*, not dropped. Forgetting to add one here does not disable
 * it — it ships it without a rate limit. A mesh makes that worse than it is
 * for a 1:1 call, since one sender's flood fans out to N-1 recipients.
 *
