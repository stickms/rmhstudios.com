...
export default function RmhCodePage() {
  ...
  const features = [
    ...
    },
    {
      ...
      description: 'Sign in with your rmhstudios.com account via browser or token flow.',
    },
    {
      icon: GitBranch,
      title: 'GitHub Auto-Repo',
      description: 'Create a GitHub repo, push code, and publish a build in one step with --create-repo.',
    },
    {
      icon: Layout,
      title: 'Tmux Workspace',
      description: 'Launch a 3-pane tmux session with rmhcode and a shell, auto-installing tmux if needed.',
    },
    {
      icon: Layers,
      title: 'MCP Integrations',
      description: 'Bundled DeepWiki and GitHub MCP servers for docs and repo interaction.',
    },
  ];
            ...
            <div className="rounded-xl border border-site-border bg-site-surface/80 backdrop-blur-sm p-6">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${
                      installTab === tab.key
                        ...
                        : 'text-site-text-muted hover:text-site-text border border-transparent'
                    }`}
                  >
              ...
              {installTab === 'mac' && (
                <div className="space-y-3">
                  ...
                  <button
                    onClick={() => setShowManualMac(!showManualMac)}
                  ...
                  {showManualMac && (
                    <CodeBlock code={MANUAL_MAC} id="manual-mac" copiedId={copiedId} onCopy={copyToClipboard} />
                  )}
                </div>
              )}
              ...
              {installTab === 'windows' && (
                <div className="space-y-4">
                  ...
                  <div>
                    <button
                      onClick={() => setShowManualWin(!showManualWin)}
                      className="flex items-center gap-1.5 text-sm text-site-text-muted hover:text-site-text transition-colors"
                    >
                    ...
                    {showManualWin && (
                      <div className="mt-2 space-y-2">
                        ...
                        <p className="text-xs text-site-text-dim">
                          Then add <code className="text-violet-400">%USERPROFILE%\.rmhcode\bin</code> to your PATH and create a <code className="text-violet-400">rmhcode.cmd</code> wrapper.
                        </p>
                      </div>
                    )}
                  </div>
              ...
              {/* Standalone Binaries Tab */}
              {installTab === 'binary' && (
                <div className="space-y-3">
                  <p className="text-sm text-site-text-muted">
                    Pre-built binaries — no Node.js required. Download, make executable (<code className="text-violet-400">chmod +x</code> on Linux/macOS), and place in your PATH.
                  </p>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-site-border bg-site-surface-hover">
                          <th className="text-left py-2.5 px-3 font-medium text-site-text text-xs">Platform</th>
                          <th className="text-left py-2.5 px-3 font-medium text-site-text text-xs">Arch</th>
                          <th className="text-left py-2.5 px-3 font-medium text-site-text text-xs">Binary</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-site-border">
                        {binaries.map((b) => (
                          <tr key={b.binary}>
                            <td className="py-2 px-3 text-site-text-muted">{b.platform}</td>
                            <td className="py-2 px-3 text-site-text-muted">{b.arch}</td>
                            <td className="py-2 px-3">
                              <a href={RELEASES_URL} target="_blank" rel="noopener noreferrer" className="font-mono text-violet-400 hover:text-violet-300 text-xs">
                                {b.binary}
                              </a>
                            </td>
                          </tr>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 space-y-12">
        ...
        <motion.div {...inViewProps}>
          <div className="rounded-xl border border-site-border bg-site-surface p-6">
            ...
            <CodeBlock
              code={`rmhcode              # launch with banner
...
rmhcode --no-banner  # suppress banner`}
            ...
            <p className="text-sm text-site-text-dim mt-3">
              Set <code className="text-violet-400 text-xs">RMHCODE_NO_BANNER=1</code> to always suppress the banner. All standard Claude Code arguments and flags work as normal.
            </p>
          </div>
        </motion.div>
          ...
          <div className="rounded-xl border border-site-border bg-site-surface p-6">
            ...
            <div className="rounded-lg border border-site-border bg-site-bg overflow-hidden mb-4">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-site-border bg-site-surface-hover">
                    <th className="text-left py-2.5 px-4 font-medium text-site-text">Provider</th>
                    <th className="text-left py-2.5 px-4 font-medium text-site-text">Flag</th>
                    <th className="text-left py-2.5 px-4 font-medium text-site-text">Requires</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-site-border">
                  {providers.map((p) => (
                    <tr key={p.name}>
                      <td className="py-2.5 px-4 text-site-text-muted">{p.name}</td>
                      <td className="py-2.5 px-4 font-mono text-violet-400 text-xs">{p.flag}</td>
                      <td className="py-2.5 px-4 font-mono text-site-text-dim text-xs">{p.requires}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <CodeBlock
              code={`rmhcode                          # Use Claude (default)
          ...
          <div className="rounded-xl border border-site-border bg-site-surface p-6">
            ...
            <div className="mb-6">
              ...
              <div className="space-y-2">
                <CodeBlock
                  code={`rmhcode login --token YOUR_TOKEN   # token flow
rmhcode login                      # browser flow`}
            ...
            {/* Publishing Example */}
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-site-text mb-2">Publishing a Build</h3>
              <div className="rounded-lg bg-black/60 border border-site-border p-4 font-mono text-sm overflow-x-auto">
                <div className="text-site-text-muted">
                  <p className="text-white">$ rmhcode push-build</p>
                  <p className="mt-2 text-site-text-dim">  Publish a new build to RMH User Builds</p>
                  <p className="text-site-text-dim">  Fields marked with * are required</p>
                  <p className="mt-2"><span className="text-violet-400">?</span> Title *: <span className="text-white">My Cool Project</span></p>
                  <p><span className="text-violet-400">?</span> Description *: <span className="text-white">A web app built with Next.js and Prisma</span></p>
                  <p><span className="text-violet-400">?</span> Repository URL: <span className="text-white">https://github.com/user/repo</span></p>
                  <p><span className="text-violet-400">?</span> Demo URL: <span className="text-white">https://my-project.vercel.app</span></p>
                  <p><span className="text-violet-400">?</span> Technologies: <span className="text-white">Next.js, TypeScript, Prisma</span></p>
                  <p><span className="text-violet-400">?</span> Tags: <span className="text-white">web, fullstack</span></p>
                  <p><span className="text-violet-400">?</span> Visibility: <span className="text-white">public</span></p>
                  <p><span className="text-violet-400">?</span> Publish now? <span className="text-white">y</span></p>
                  <p className="mt-2"><span className="text-green-400">✓</span> Build &quot;My Cool Project&quot; published!</p>
                  <p className="text-site-text-dim">  View at: https://rmhstudios.com/user-builds/my-cool-project</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <p className="text-sm text-site-text-muted mb-3">
                Use <code className="text-violet-400">--create-repo</code> to create a GitHub repository, push your code, and publish — all in one step:
              </p>
              ...
              <p className="text-xs text-site-text-dim mt-2">
                Requires <code className="text-violet-400">GITHUB_PERSONAL_ACCESS_TOKEN</code> in your environment with the <code className="text-violet-400">repo</code> scope.
              </p>
            </div>
            ...
            <div>
              ...
              <CodeBlock
                code={`rmhcode list-builds              # interactive menu to browse your builds
rmhcode edit-build my-project    # edit a build by its slug`}
              ...
              />
              <p className="text-xs text-site-text-dim mt-2">
                Select a build from the interactive menu to edit fields, publish/unpublish, or view the URL — all without leaving the terminal.
              </p>
            </div>
          </div>
        </motion.div>
...
+──────────────+──────────────+`}
            ...
            <CodeBlock
              code={`rmhcode --tmux                    # launch workspace
rmhcode --tmux --provider gemini  # all panes use Gemini`}
            ...
            />
            <p className="text-xs text-site-text-dim mt-2">
              Auto-installs tmux via Homebrew (macOS) or your Linux package manager if missing. Reattaches to an existing session if one is running.
            </p>
          </div>
        </motion.div>
        ...
        <motion.div {...inViewProps}>
          <div className="rounded-xl border border-site-border bg-site-surface p-6">
            ...
            <p className="text-sm text-site-text-muted mb-4">
              Running <code className="text-violet-400">rmhcode --init</code> in any project directory scans your codebase and generates a <code className="text-violet-400">CLAUDE.md</code> file with:
            </p>
        ...
        <motion.div {...inViewProps}>
          <div className="rounded-xl border border-site-border bg-site-surface p-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="p-4 rounded-lg bg-site-bg border border-site-border">
                <h3 className="font-semibold text-site-text mb-1">DeepWiki</h3>
                <p className="text-sm text-site-text-muted">
                  AI-powered documentation for any public GitHub repository. Browse wiki structures, read full docs, or ask questions grounded in source code.
                </p>
              </div>
              <div className="p-4 rounded-lg bg-site-bg border border-site-border">
                <h3 className="font-semibold text-site-text mb-1">GitHub MCP</h3>
                <p className="text-sm text-site-text-muted">
                  Interact with GitHub issues, pull requests, and repositories directly from the CLI.
                </p>
              </div>
            </div>
            <p className="text-xs text-site-text-dim mt-3">
              To use GitHub MCP&apos;s full capabilities, set <code className="text-violet-400">GITHUB_PERSONAL_ACCESS_TOKEN</code> in your environment.
            </p>
          </div>
        </motion.div>
          ...
          <div className="rounded-xl border border-site-border bg-site-surface overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-site-border">
                {cliCommands.map((cmd) => (
                  <tr key={cmd.command}>
                    <td className="py-3 px-4 font-mono text-violet-400 text-xs sm:text-sm whitespace-nowrap">{cmd.command}</td>
                    <td className="py-3 px-4 text-site-text-muted">{cmd.description}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <motion.div {...inViewProps}>
          <div className="rounded-xl border border-site-border bg-site-surface p-5">
            <h2 className="text-lg font-semibold text-site-text mb-4 flex items-center gap-2">
              <Trash2 className="w-5 h-5 text-site-text-muted" />
              Uninstall
            </h2>
            <div className="space-y-4">
              <div>
                <p className="text-xs text-site-text-dim font-medium uppercase tracking-wide mb-2">macOS / Linux</p>
                <CodeBlock code={UNINSTALL_MAC} id="uninstall-mac" copiedId={copiedId} onCopy={copyToClipboard} />
              </div>
              <div>
                <p className="text-xs text-site-text-dim font-medium uppercase tracking-wide mb-2">Windows (PowerShell install)</p>
                <CodeBlock code={UNINSTALL_WIN} id="uninstall-win" copiedId={copiedId} onCopy={copyToClipboard} />
              </div>
              <p className="text-xs text-site-text-dim">
