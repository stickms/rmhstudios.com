...
function AuthContent() {
  ...
  async function handleAuthorize() {
    if (!callback || !sessionId) { setError('Missing callback or session parameters.'); return; }
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json(); throw new Error(data.error || 'Failed to generate token'); }
    ...
    } catch (e) { setError(e instanceof Error ? e.message : 'Authorization failed'); setAuthorizing(false); }
  }
      ...
      <div className="min-h-screen bg-site-bg flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md w-full">
          <div className="rounded-xl border border-site-border bg-site-surface p-6 text-center">
            ...
            <h1 className="text-xl font-semibold text-site-text mb-2">Sign in to authorize rmhcode</h1>
            <p className="text-sm text-site-text-muted mb-6">You need to sign in to your RMH account to authorize the CLI.</p>
            <Link to={loginUrl}><Button variant="accent" className="w-full bg-violet-600 hover:bg-violet-500">Sign In to Continue</Button></Link>
          </div>
        </motion.div>
      </div>
    );
      ...
      <div className="min-h-screen bg-site-bg flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md w-full">
          <div className="rounded-xl border border-red-500/30 bg-red-500/5 p-6 text-center">
            ...
            <h1 className="text-xl font-semibold text-site-text mb-2">Invalid Authorization Request</h1>
            <p className="text-sm text-site-text-muted mb-6">This page should be opened from the rmhcode CLI. Missing required parameters.</p>
            <Link to="/rmhcode"><Button variant="secondary">Go to rmhcode</Button></Link>
          </div>
        </motion.div>
      </div>
    );
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-md w-full">
        <div className="rounded-xl border border-site-border bg-site-surface p-6">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div><h1 className="text-xl font-semibold text-site-text">Authorize rmhcode</h1><p className="text-sm text-site-text-muted">CLI is requesting access to your account</p></div>
          </div>
          <div className="p-4 rounded-lg bg-site-bg border border-site-border mb-6">
            <div className="flex items-center gap-3">
              {session.user.image ? <img src={session.user.image} alt={session.user.name || 'User'} className="w-10 h-10 rounded-full" /> : <div className="w-10 h-10 rounded-full bg-violet-500/20 flex items-center justify-center text-violet-400 font-bold">{(session.user.name?.[0] || 'U').toUpperCase()}</div>}
          ...
          <div className="mb-6">
            <h2 className="text-sm font-medium text-site-text mb-3 flex items-center gap-2"><Shield className="w-4 h-4 text-violet-400" />This will allow rmhcode to:</h2>
            <ul className="space-y-2 text-sm text-site-text-muted">
              <li className="flex items-center gap-2"><Check className="w-4 h-4 text-green-400" />Access your account information</li>
              <li className="flex items-center gap-2"><Check className="w-4 h-4 text-green-400" />Create and manage your User Builds</li>
              <li className="flex items-center gap-2"><Check className="w-4 h-4 text-green-400" />Publish projects on your behalf</li>
            </ul>
          </div>
          {error && <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/30 text-sm text-red-400 mb-6">{error}</div>}
          <div className="flex gap-3">
            <Button onClick={handleDeny} variant="secondary" className="flex-1" disabled={authorizing}>Deny</Button>
            <Button onClick={handleAuthorize} variant="accent" className="flex-1 bg-violet-600 hover:bg-violet-500" disabled={authorizing || !!error}>
              {authorizing ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Authorize'}
            </Button>
          </div>
          <p className="text-xs text-site-text-dim text-center mt-4">Token expires in 30 days. You can revoke access anytime from your account settings.</p>
        </div>
      </motion.div>
    </div>
  );
