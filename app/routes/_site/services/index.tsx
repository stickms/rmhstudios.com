...
const SERVICES: ServiceDef[] = [
  ...
  },
  {
    ...
    ctaKey: 'services-restaurant-cta',
    cta: 'Read the menu',
    features: [
      ['services-restaurant-f1', 'Nine courses, every one of them a sphere'],
      ['services-restaurant-f2', 'Each pearl drawn at the diameter it is served at'],
      ['services-restaurant-f3', 'Two seatings a night, Wednesday to Saturday'],
      ['services-restaurant-f4', 'Two stars, one of them for the lighting'],
    ],
  },
  {
    id: 'cars',
...
function ServiceSummaryPanel({ service }: { service: ServiceSummary }) {
  ...
  // The three original verticals reuse the sidebar nav string they already ship
  // in every locale, which is why a computed key is safe for them. The
  // restaurant has no nav entry, so it carries a key of its own in `site` — and
  // that one has to be spelled out, because `i18next-parser` cannot see through
  // a computed key and it would never reach `locales/` otherwise.
  const name =
