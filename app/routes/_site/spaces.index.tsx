...
// SSR the live-spaces list so the directory is present at first paint.
const fetchLiveSpaces = createServerFn({ method: 'GET' }).handler(async () => {
  const request = getRequest();
  await auth.api.getSession({ headers: request.headers }).catch(() => null);
  return { spaces: await listLiveSpaces() };
});
...
function SpacesIndexPage() {
  ...
  return (
    <>
      ...
      >
        <MobileTopBar title={t('spaces-title', { defaultValue: 'Spaces' })} />
