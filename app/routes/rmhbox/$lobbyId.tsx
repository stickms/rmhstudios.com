...
function LobbyPage() {
  ...
  const backlinkLabel = t("leave", { defaultValue: "Leave" });
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        ...
        if (mounted) toast.error(err instanceof Error ? err.message : t("failed-to-connect", { defaultValue: "Failed to connect" }));
      }
    }
        ...
        <div className="flex items-center justify-center h-full">
          <div className="text-9xl font-bold animate-pulse text-(--rmhbox-accent)" style={{ fontFamily: 'var(--rmhbox-font-display)' }}>
            {/* Read from the centralized timer (TIMER_START/TICK), not
                ...
                countdownValue fallback if a sync lands mid-countdown. */}
...
export const Route = createFileRoute('/rmhbox/$lobbyId')({
  component: LobbyPage,
});
