...
export const Route = createFileRoute('/api/signal-forge/save')({
  server: {
    handlers: {
      POST: defineHandler(
        { auth: 'none', rateLimit: { limit: 10, windowMs: 60_000, prefix: 'signal-forge-save' } },
        async ({ request }) => {
          ...
          try {
            ...
            const newHighScore = existing
              ? Math.max(existing.highScore, currentScore)
              : currentScore;
            const newFloorReached = existing
              ? Math.max(existing.floorReached, currentFloor)
              : currentFloor;
