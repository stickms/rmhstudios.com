        ...
        for (const application of applications) {
          ...
          if (application.followUpDate)
            add(
              ...
              `Follow up: ${label}`,
              application.notes ?? 'RMHLadder follow-up reminder',
            );
          for (const [index, interview] of application.interviewDates.entries()) {
            add(
              `interview-${application.id}-${index}`,
              interview,
              `Interview: ${label}`,
              'RMHLadder interview reminder',
            );
          }
          if (application.job.applicationDeadline) {
            add(
              `deadline-${application.job.id}`,
              application.job.applicationDeadline,
              `Deadline: ${label}`,
              application.job.originalPostingUrl,
            );
          }
        }
        const calendar = [
          'BEGIN:VCALENDAR',
          'VERSION:2.0',
          'PRODID:-//RMH Studios//RMHLadder//EN',
          ...events,
          'END:VCALENDAR',
          '',
        ].join('\r\n');
