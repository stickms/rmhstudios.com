      ...
      POST: async ({ request, params }) => {
        try {
          ...
          const { allowed, retryAfter } = rateLimit(`${session.user.id}:${getClientIp(request)}`, {
            limit: 10,
            windowMs: 60 * 60_000,
            prefix: 'rmhladder-resume-confirm',
          });
          if (!allowed)
            return Response.json(
              { error: 'Too many match refreshes.' },
              { status: 429, headers: { 'Retry-After': String(retryAfter) } },
            );
          const parsed = bodySchema.safeParse(await request.json().catch(() => null));
          if (!parsed.success)
            return Response.json(
              { error: 'Review the extracted profile before confirming it.' },
              { status: 400 },
            );
          return Response.json(
            await confirmResumeProfile(resumePrisma, {
          ...
          if (!/not found|invalid/i.test(message))
            console.error('[rmhladder-resume] profile confirmation failed:', error);
          return Response.json(
            {
