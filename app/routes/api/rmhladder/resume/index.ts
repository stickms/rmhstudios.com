    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          if (!(file instanceof File) || file.size === 0) return Response.json({ error: 'Choose a PDF or DOCX resume.' }, { status: 400 });
          if (file.size > RESUME_MAX_BYTES) return Response.json({ error: 'Resume exceeds the 10 MiB limit.' }, { status: 413 });
          const result = await uploadResume(resumePrisma, {
            userId: session.user.id,
            resumeId: typeof form.get('resumeId') === 'string' ? String(form.get('resumeId')) : undefined,
          ...
          const userError = /empty|limit|PDF|DOCX|filename|not found|supported|document body|parse/i.test(message);
          if (!userError) console.error('[rmhladder-resume] upload failed:', error);
          return Response.json({ error: userError ? message : 'Resume upload failed.' }, { status: userError ? 400 : 500 });
        }
      },
    },
  },
