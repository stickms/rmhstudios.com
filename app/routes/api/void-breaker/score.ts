...
export const APIRoute = createAPIFileRoute("/api/void-breaker/score")({
  POST: async ({ request }) => {
  const ip = getClientIp(request);
  const { allowed, retryAfter } = rateLimit(ip, { limit: 5, windowMs: 60_000, prefix: 'void-breaker-score' });
  if (!allowed) {
  ...
  try {
    ...
    const conflict = await prisma.voidBreakerPlayer.findUnique({ where: { username: cleanUsername } });
    if (conflict) {
