...
export const Route = createFileRoute('/api/discord/daily-progress')({
  server: {
    handlers: {
      ...
      POST: defineHandler(
        { auth: 'none', rateLimit: { limit: 120, windowMs: 60_000, prefix: 'daily-progress' } },
        async ({ request }) => {
          try {
            const body = await request.json();
            const { accessToken, dateKey, gridJson, moves, completed, ratingLabel, ratingEmoji } =
              body;

            const discordId = await resolveDiscordId(
              tokenFromRequest(request, undefined, accessToken),
            );
            if (!discordId) {
              return Response.json({ error: 'Invalid or missing Discord token' }, { status: 401 });
            }
