...
async function verifyDiscordIdentity(
  ...
  try {
    ...
    const identity = {
      id: u.id as string,
      username: (u.global_name ?? u.username ?? null) as string | null,
    };
...
const SITE_URL = stripTrailingSlash(
  process.env.SITE_URL ?? process.env.VITE_BETTER_AUTH_URL ?? 'https://rmhstudios.com',
);
...
function buildEmbed(dateKey: string, guildId: string, participants: ParticipantRow[]): object {
  ...
  for (const p of completed) {
    lines.push(
      `${p.ratingEmoji ?? '\u{1F4A1}'} **${p.username}** \u2014 ${p.moves} move${p.moves !== 1 ? 's' : ''}${p.ratingLabel ? ` (${p.ratingLabel})` : ''}`,
    );
  }
    ...
    embeds: [
      {
        ...
        description: [`**${shapeLabel}**`, '', ...lines].join('\n'),
        color: 0xf59e0b,
        image: { url: imgUrl },
...
function buildRaceEmbed(raceResults: {
  roundNumber: number;
  raceMode: string;
...
async function discordApi(path: string, method: string, body?: object): Promise<any> {
  ...
  if (!token) {
    console.warn(
      '[embed] No bot token set (DISCORD_ACTIVITY_BOT_TOKEN or DISCORD_BOT_TOKEN) — skipping Discord API call',
    );
    return null;
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (status === 403 || status === 404) {
        console.warn(
          `Discord API ${method} ${path}: ${status} (bot lacks access) — ${text.slice(0, 200)}`,
        );
...
async function postOrEditEmbed(
  ...
  if (cached) {
    ...
    if (age < EMBED_FRESHNESS_MS) {
      ...
      console.log(
        `[embed] Editing recent message ${cached.messageId} in channel ${channelId} (${Math.round(age / 1000)}s old)`,
      );
      ...
      console.log(
        `[embed] Replying to stale message ${cached.messageId} in channel ${channelId} (${Math.round(age / 1000)}s old)`,
      );
      const replyBody = {
        ...embed,
  ...
  console.log(
    `[embed] Posting new embed to channel ${channelId} (guild ${guildId}, ${participants.length} participants)`,
  );
...
export const Route = createFileRoute('/api/discord/embed')({
  server: {
    handlers: {
      ...
      POST: defineHandler(
        { auth: 'none', rateLimit: { limit: 30, windowMs: 60_000, prefix: 'discord-embed' } },
        async ({ request }) => {
          try {
            ...
            const channelId =
              typeof body.channelId === 'string'
                ? body.channelId.replace(/"/g, '')
                : body.channelId;
            const guildId =
              typeof body.guildId === 'string' ? body.guildId.replace(/"/g, '') : body.guildId;

            console.log(
              `[embed] action=${action} guild=${guildId} channel=${channelId} user=${user?.id} dateKey=${dateKey}`,
            );
            ...
            if (!channelId) {
              console.log(
                '[embed] Warning: guildId present but channelId is null — will track in DB but cannot post embed',
              );
            }
            ...
            if (action === 'race_completed') {
              ...
              if (!blockedChannels.has(channelId)) {
                ...
                if (cached?.messageId) {
                  ...
                  console.log(
                    `[embed] Editing race message ${cached.messageId} with round ${roundNumber} results`,
                  );
                  const result = await discordApi(
                    `/channels/${channelId}/messages/${cached.messageId}`,
                    'PATCH',
                  ...
                  if (result) {
                    ...
                    if (!blockedChannels.has(channelId)) {
                      ...
                      if (msg?.id) {
                        console.log(
                          `[embed] Posted fresh race results (round ${roundNumber}) as message ${msg.id}`,
                        );
                        messageCache.set(sessionCacheKey, {
                          messageId: msg.id,
                          updatedAt: Date.now(),
                        });
                      }
                  ...
                  if (msg?.id) {
                    console.log(
                      `[embed] Posted race results (round ${roundNumber}) as message ${msg.id}`,
                    );
            ...
            if (channelId) {
              ...
              postOrEditEmbed(channelId, guildId, dateKey, guildParticipants).catch((e) =>
                console.error('[embed] postOrEditEmbed error:', e),
              );
            }
