...
function checkRaceComplete(lobby: LobbyEntry): void {
  ...
  const racers = lobby.participants.filter(
    (p) => p.status === 'solving' || p.status === 'solved' || p.status === 'dnf',
  );
    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        if (
          ...
          now >= lobby.countdownStartedAt + COUNTDOWN_MS
        ) {
          lobby.phase = 'racing';
      ...
      POST: defineHandler(
        { auth: 'none', rateLimit: { limit: 90, windowMs: 60_000, prefix: 'discord-race' } },
        async ({ request }) => {
          try {
            ...
            switch (action) {
              ...
              case 'start': {
                ...
                const lobby = lobbies.get(instanceId);
                if (!lobby || lobby.phase !== 'waiting' || lobby.hostId !== discordId)
                  return Response.json({ error: 'Invalid' }, { status: 400 });
                const host = lobby.participants.find((p) => p.discordId === discordId);
                if (!host?.ready)
                  return Response.json({ error: 'Host must be ready' }, { status: 400 });
                lobby.roundNumber += 1;
              ...
              case 'update': {
                ...
                const lobby = lobbies.get(instanceId);
                if (!lobby || (lobby.phase !== 'racing' && lobby.phase !== 'countdown'))
                  return Response.json({ error: 'Invalid' }, { status: 400 });
                const p = lobby.participants.find((x) => x.discordId === discordId);
                if (p && p.status !== 'idle') {
                  if (status === 'solving' || status === 'solved' || status === 'dnf')
                    p.status = status;
              ...
              case 'leave': {
                ...
                if (lobby.hostId === discordId) {
                  const e = lobby.participants.reduce((a, b) => (a.joinedAt < b.joinedAt ? a : b));
                  lobby.hostId = e.discordId;
                }
                checkRaceComplete(lobby);
                lobby.lastUpdated = now;
                return Response.json({ success: true });
              ...
              case 'set_mode': {
                ...
                if (!lobby || lobby.phase !== 'waiting' || lobby.hostId !== discordId)
                  return Response.json({ error: 'Invalid' }, { status: 400 });
                if (raceMode !== 'time' && raceMode !== 'moves')
              ...
              case 'return_to_lobby': {
                const { discordId } = body;
                const lobby = lobbies.get(instanceId);
                if (!lobby || lobby.phase !== 'results' || lobby.hostId !== discordId)
                  return Response.json({ error: 'Invalid' }, { status: 400 });
                lobby.phase = 'waiting';
                lobby.seed = null;
