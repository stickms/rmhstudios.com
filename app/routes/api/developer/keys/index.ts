 ...
 * POST /api/developer/keys — create a key (subscriber only). The plaintext is
    ...
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          return Response.json({ keys, hasApiAccess: hasApiAccess(tier), tier });
        } catch (error) {
      ...
      POST: async ({ request }) => {
        try {
          ...
          const key = await prisma.developerApiKey.create({
            data: { userId, name: parsed.data.name.trim(), prefix, hashedKey },
