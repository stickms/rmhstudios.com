        ...
        withDeveloperApi(
          ...
          async ({ userId, tier, json, error }) => {
            ...
            if (!hasApiImageUpload(tier)) {
              return error("feature_not_available", "Image upload requires a Starter plan or higher.", 403);
            }

            // 2. Dedicated tight per-key limit (far below the generic 120/min).
            const burst = await keyedLimit(`dev-api-image:${userId}`, 15, 60_000);
            if (!burst.allowed) {
              return error("rate_limited", "Too many uploads. Slow down.", 429, { "Retry-After": String(burst.retryAfter) });
            }

            // 3. Tier-scaled daily quota.
            const quota = await checkDailyUploadQuota({ limit: keyedLimit }, { userId, tier });
            if (!quota.allowed) {
              return error("quota_exceeded", "Daily image upload limit reached.", 429, { "Retry-After": String(quota.retryAfter) });
            }

            // 4. Reject oversize bodies before reading them into memory.
            const declared = Number(request.headers.get("content-length") ?? "0");
            if (declared > MEDIA_MAX_BYTES) {
              return error("payload_too_large", `Image too large. Maximum size is ${MEDIA_MAX_BYTES / 1024 / 1024} MB.`, 413);
            }
            ...
            try {
              ...
              return error("invalid_request", "Expected multipart/form-data with an `image` field.", 400);
            }
            const file = form.get("image");
            if (!(file instanceof File) || file.size === 0) {
              return error("invalid_request", "No image provided. Send one file in the `image` field.", 400);
            }
            if (file.size > MEDIA_MAX_BYTES) {
              return error("invalid_request", `Image too large. Maximum size is ${MEDIA_MAX_BYTES / 1024 / 1024} MB.`, 400);
            }
