...
function aggregatePayout(grossFaresCents: number, tipsCents: number, trips: number) {
  ...
  const insuranceCents = Math.min(
    ...
    Math.max(
      trips * PAYOUT_RATES.insuranceMinCents,
      Math.round(grossFaresCents * PAYOUT_RATES.insuranceRate),
    ),
  );
    ...
    handlers: {
      GET: defineHandler({}, async ({ session }) => {
        ...
        return Response.json({
          ...
          recent: recent.map((t) => ({
            ...t,
            driverEarningsCents: payoutBreakdown(t.estimatedFareCents, t.tipCents)
              .driverEarningsCents,
          })),
        });
      }),
