      ...
      POST: defineHandler({}, async ({ request, params, session }) => {
        ...
        if (action === 'accept') {
          ...
          await notifyRider(
            ...
            userId,
            ride,
            `${me?.name ?? 'Your driver'} accepted your ride and is on the way.`,
          );
        } else if (action === 'start') {
          if (ride.driverId !== userId) {
            return Response.json(
              { error: 'Only the assigned driver can do that.' },
              { status: 403 },
            );
          }
          const result = await prisma.ride.updateMany({
          ...
          await notifyRider(
            ...
            userId,
            ride,
            'Your trip is complete. Tap to rate your driver.',
          );
          ...
          if (isRider && ride.driverId) {
            ...
            await notifyRider(
              ...
              userId,
              ride,
              'Your driver cancelled — request a new ride anytime.',
            );
          }
        } else if (action === 'tip') {
          if (ride.riderId !== userId) {
