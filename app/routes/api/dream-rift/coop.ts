...
export const Route = createFileRoute('/api/dream-rift/coop')({
  server: {
    handlers: {
      ...
      GET: defineHandler(
        { auth: 'none', rateLimit: { limit: 20, windowMs: 60_000, prefix: 'dream-rift-coop-get' } },
        async ({ request }) => {
          try {
      ...
      POST: defineHandler(
        { auth: 'none', rateLimit: { limit: 5, windowMs: 60_000, prefix: 'dream-rift-coop-post' } },
        async ({ request }) => {
          try {
            ...
            const { difficulty, combinedScore, timeSurvived, stageReached, cleared, players } =
              body;

            const difficultyKey: Difficulty =
              typeof difficulty === 'string' &&
              (DIFFICULTIES as readonly string[]).includes(difficulty.toLowerCase())
            ...
            if (
              ...
              combinedScore < 0 ||
              combinedScore > MAX_COMBINED_SCORE
            ) {
              return Response.json({ error: 'Invalid combined score' }, { status: 400 });
            }
            if (
              typeof timeSurvived !== 'number' ||
              timeSurvived < 0 ||
              timeSurvived > MAX_TIME_SURVIVED
            ) {
              ...
              return {
                ...
                score:
                  typeof obj.score === 'number' && obj.score >= 0
                    ? Math.round(Math.min(obj.score, 10_000_000))
                    : 0,
                userId:
                  typeof obj.userId === 'string' && obj.userId.length > 0 && obj.userId !== 'local'
                    ? obj.userId
                    : null,
              };
            });
            ...
            const ids = [
              ...new Set(rawPlayers.map((p) => p.userId).filter((v): v is string => !!v)),
            ];
            ...
            const safeStage =
              typeof stageReached === 'number' && stageReached >= 1 ? Math.round(stageReached) : 1;
