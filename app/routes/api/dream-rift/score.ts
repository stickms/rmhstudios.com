...
export const Route = createFileRoute('/api/dream-rift/score')({
  server: {
    handlers: {
      POST: defineHandler(
        { auth: 'none', rateLimit: { limit: 5, windowMs: 60_000, prefix: 'dream-rift-score' } },
        async ({ request }) => {
          try {
            const { score, difficulty, stage, character, graze, spellsCaptured } =
              await request.json();
            ...
            const difficultyKey: Difficulty =
              typeof difficulty === 'string' && difficulty.toLowerCase() in DIFFICULTY_FIELDS
            ...
            const safeSpells =
              typeof spellsCaptured === 'number' && spellsCaptured >= 0
                ? Math.round(spellsCaptured)
                : 0;
