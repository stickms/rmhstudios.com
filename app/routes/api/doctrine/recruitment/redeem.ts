...
export const Route = createFileRoute('/api/doctrine/recruitment/redeem')({
  server: {
    handlers: {
      POST: defineHandler(
        { auth: 'none', rateLimit: { limit: 5, windowMs: 300_000, prefix: 'doctrine-redeem' } },
        async ({ request }) => {
          try {
            ...
            if (recruitment.uses >= recruitment.maxUses) {
              return Response.json(
                { error: 'Recruitment code has reached maximum uses' },
                { status: 410 },
              );
            }
            ...
            if (recruitment.recruiterId === session.user.id) {
              return Response.json(
                { error: 'Cannot redeem your own recruitment code' },
                { status: 400 },
              );
            }
            ...
            if (user?.doctrineRecruitedById) {
              return Response.json(
                { error: 'Already recruited by another operator' },
                { status: 409 },
              );
            }
            ...
            const outcome = await prisma
              ...
              .catch((err) => {
                if (err instanceof Error && err.message === 'CODE_EXHAUSTED')
                  return 'CODE_EXHAUSTED' as const;
                throw err;
              });
