  ...
  server: {
    handlers: {
      POST: defineHandler(
        {
          auth: 'none',
          rateLimit: { limit: 10, windowMs: 60_000, prefix: 'doctrine-puzzle-submit' },
        },
        async ({ request }) => {
          try {
            ...
            if (new Date() > puzzle.resetsAt) {
              return Response.json(
                { error: 'Puzzle has expired. Time waits for no one.' },
                { status: 410 },
              );
            }
            ...
            if (puzzle.isSahur) {
              ...
              if (!isSahurActive(user?.doctrineTimezone ?? 'America/New_York')) {
                return Response.json(
                  { error: 'This puzzle is only available during Sahur Mode (3-4 AM).' },
                  { status: 403 },
                );
              }
            }
            ...
            if (existing) {
              return Response.json(
                { error: 'Already submitted. No second chances. TUNG TUNG TUNG.' },
                { status: 409 },
              );
            }
            ...
            // Validate and score
            const puzzleData = {
              mode: puzzle.mode.toLowerCase() as
                'alibi' | 'spectrum' | 'outcast' | 'chainlink' | 'impostor',
              seed: puzzle.seed,
