...
import {
  ...
  calculateScore,
  stripSolution,
} from '@/lib/doctrine/puzzle-engine';
...
export const Route = createFileRoute('/api/doctrine/puzzles/replay')({
  server: {
    handlers: {
       ...
       */
      GET: defineHandler(
        { auth: 'none', rateLimit: { limit: 20, windowMs: 60_000, prefix: 'doctrine-replay-gen' } },
        async ({ request }) => {
          try {
      ...
      POST: defineHandler(
        {
          auth: 'none',
          rateLimit: { limit: 15, windowMs: 60_000, prefix: 'doctrine-replay-submit' },
        },
        async ({ request }) => {
          try {
            const session = await auth.api.getSession({ headers: request.headers });
            ...
            return Response.json({
              success: true,
              submission: { correct, score, xpMultiplier: 1 },
            });
          } catch (e) {
