    ...
    handlers: {
      POST: defineHandler({ auth: 'optional' }, async ({ request, session }) => {
        ...
        if (!s3Configured() && process.env.LIBRARY_ALLOW_LOCAL_STORAGE !== 'true') {
          console.error(
            'Library upload refused: object storage (S3_*) is not configured — uploads would not persist.',
          );
        ...
        const deps: UploadDeps = {
          ...
          countUserDocs: (uid) =>
            prisma.libraryDocument.count({ where: { uploadedByUserId: uid } }),
        ...
        if (!stored) {
          console.error(
            `Library upload ${id}: stored object not found after write — rolling back.`,
          );
