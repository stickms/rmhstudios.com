...
import { Sohumbum2Page } from '@/components/sohumbum2/Sohumbum2Page';
import sohumbum2Css from '@/components/sohumbum2/sohumbum2.css?url';
import { buildMeta } from '@/lib/seo';
import { getWatchState } from '@/lib/sohumbum2/activity.server';
import { DEFAULT_HISTORY_DAYS, formatCount, formatDuration } from '@/lib/sohumbum2/config';
 ...
 * it wants the full-screen tier and no radial shell; `sohumbum2` is registered
 * in `FULLSCREEN_ROUTE_SEGMENTS` / `FULLSCREEN_TIER_DIRS`
 * (`lib/__tests__/design-consistency.test.ts`), which is how this repo says
 * "this surface has its own palette". It is a DIRECTORY (`sohumbum2/index.tsx`
 * plus `sohumbum2/$date.tsx`) rather than two flat files because that gate
 * matches on the third path segment — `sohumbum2.index.tsx` would present as
 * segment `sohumbum2.index` and be held to the site contract it does not use.
 *
 ...
 * are real); the component then polls `/api/sohumbum2/activity`.
 */
...
export const Route = createFileRoute('/sohumbum2/')({
  ...
  component: Sohumbum2Route,
});

function Sohumbum2Route() {
  const state = Route.useLoaderData();
  return <Sohumbum2Page initialState={state} historyDays={DEFAULT_HISTORY_DAYS} />;
}
