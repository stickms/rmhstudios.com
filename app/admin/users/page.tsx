...
import { ArrowLeft, Loader2, Search, CheckCircle, Shield, AlertCircle } from 'lucide-react';
...
export default function AdminUsersPage() {
    ...
    const loadMore = async () => {
        ...
        try {
            const url = search 
                ? `/api/admin/users?q=${encodeURIComponent(search)}&cursor=${nextCursor}` 
                : `/api/admin/users?cursor=${nextCursor}`;
    ...
    const toggleStatus = async (userId: string, field: 'isAdmin' | 'isVerified', currentValue: boolean) => {
        if (field === 'isAdmin' && !currentValue && !confirm("Are you sure you want to grant Admin privileges to this user? They will have full access to the database.")) {
            return;
        }

        if (field === 'isAdmin' && currentValue && userId === session?.user?.id) {
            alert("You cannot remove your own admin privileges from the dashboard.");
            return;
        }
                                    ...
                                    <div className="w-10 h-10 rounded-full bg-site-bg overflow-hidden flex-shrink-0 border border-site-border">
                                        {user.image ? (
                                            ...
                                            <div className="w-full h-full bg-linear-to-br from-site-surface to-site-surface-hover flex items-center justify-center">
                                                <span className="text-xs font-bold text-site-text-dim">{user.name?.charAt(0) || user.username?.charAt(0) || 'U'}</span>
                                            </div>
                                        )}
                                    </div>
                                    <div className="min-w-0">
                                        <div className="flex items-center gap-1.5">
                                            <Link href={`/profile/${user.username}`} className="font-semibold text-site-text hover:text-site-accent truncate">
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center sm:gap-2 text-sm text-site-text-dim truncate">
