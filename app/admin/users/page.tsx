...
export default function AdminUsersPage() {
    ...
    const startEditHandle = (user: User) => {
        setEditingHandle(user.id);
    ...
    const saveHandle = async (userId: string) => {
        ...
        try {
            const res = await fetch('/api/admin/users', {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId, handle: handleInput.trim().toLowerCase() })
            });
            ...
            setHandleError('Failed to connect to server');
        } finally {
            setUpdating(null);
        }
    };
